...
- `runtime/` 包含节点代理、本地管理前端、工程运行组件与页面 SDK；工业采集代码统一位于根目录 `collector/`，由 NodeAgent 托管原生进程。
...
| 工业协议采集与 WAL              | `collector/README.md`、`docs/02-系统设计/工业采集架构.md`   |
...
- Collector：按 `collector/README.md` 选择 .NET 测试项目或根 `collector:*` 脚本；节点托管逻辑仍按 NodeAgent 模块验证。
