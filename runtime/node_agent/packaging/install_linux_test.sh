...
printf 'fake foundation images\n' > "$PACKAGE_DIR/k3s/$ARCH/induforge-foundation-images-$ARCH.tar.gz"
...
FOUNDATION_SUM="$(shasum -a 256 "$PACKAGE_DIR/k3s/$ARCH/induforge-foundation-images-$ARCH.tar.gz" | awk '{print $1}')"
printf '%s  k3s\n%s  k3s-airgap-images-%s.tar.zst\n%s  induforge-foundation-images-%s.tar.gz\n' "$K3S_SUM" "$IMAGES_SUM" "$ARCH" "$FOUNDATION_SUM" "$ARCH" > "$PACKAGE_DIR/k3s/$ARCH/SHA256SUMS"
...
test -f "$PREFIX/k3s/$ARCH/induforge-foundation-images-$ARCH.tar.gz"
