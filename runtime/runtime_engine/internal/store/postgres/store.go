...
func (s *Store) verifySchemaContract(ctx context.Context) error {
	expected := map[string][]schemaColumn{
		...
		"compute_command_audit":  {{"deployment_id", "text", true}, {"command_id", "uuid", true}, {"compute_id", "uuid", true}, {"requested_by", "text", true}, {"requested_at", "timestamp with time zone", true}, {"binding_epoch", "bigint", true}, {"idempotency_key", "text", true}, {"status", "text", true}, {"result_event_ids", "jsonb", true}, {"result_version", "bigint", false}, {"failure_code", "text", false}, {"created_at", "timestamp with time zone", true}, {"updated_at", "timestamp with time zone", true}},
		...
		"alarm_ack_audit":        {{"id", "bigint", true}, {"deployment_id", "text", true}, {"alarm_item_id", "uuid", true}, {"subject_id", "text", true}, {"comment", "text", true}, {"acknowledged_at", "timestamp with time zone", true}, {"state_version", "bigint", true}},
	}
	defaults := map[string]map[string]string{"schema_meta": {"applied_at": "now()"}, "role_fence": {"activated_at": "now()"}, "producer_fence": {"activated_at": "now()"}, "processed_event": {"delivery_metadata": "'{}'::jsonb", "processed_at": "now()"}, "consumer_checkpoint": {"updated_at": "now()"}, "transactional_outbox": {"headers": "'{}'::jsonb", "state": "'pending'::text", "attempt_count": "0", "next_attempt_at": "now()", "created_at": "now()", "updated_at": "now()"}, "processing_failure": {"quarantined_at": "now()"}, "producer_sequence": {"updated_at": "now()"}, "point_current": {"updated_at": "now()"}, "compute_input_snapshot": {"updated_at": "now()"}, "compute_command_audit": {"result_event_ids": "'[]'::jsonb", "created_at": "now()", "updated_at": "now()"}, "compute_trigger_state": {"previous_seen": "false", "condition_active": "false", "updated_at": "now()"}, "compute_schedule_state": {"run_count": "0", "updated_at": "now()"}, "alarm_item_state": {"updated_at": "now()"}, "alarm_ack_audit": {"comment": "''::text"}}
	identities := map[string]string{"transactional_outbox.id": "a", "processing_failure.id": "a", "alarm_ack_audit.id": "a"}
...
func (s *Store) verifySchemaDefinitions(ctx context.Context) error {
	...
	required := map[string][]string{
		...
		"compute_command_audit":  {"PRIMARY KEY (deployment_id, command_id)", "UNIQUE (deployment_id, idempotency_key)", "binding_epoch >= 1", "length(requested_by)", "length(idempotency_key)", "status = ANY", "jsonb_typeof(result_event_ids)", "failure_code IS NOT NULL", "result_version IS NULL OR result_version > 0"},
		...
		"alarm_ack_audit":        {"PRIMARY KEY (id)", "length(subject_id) > 0", "length(comment) <= 2048", "state_version > 0"},
	}
	expectedConstraintCount := map[string]int{"schema_meta": 2, "role_fence": 6, "producer_fence": 6, "processed_event": 7, "consumer_checkpoint": 5, "transactional_outbox": 13, "processing_failure": 12, "producer_sequence": 6, "point_history": 8, "point_current": 9, "compute_input_snapshot": 9, "compute_command_audit": 10, "compute_trigger_state": 7, "compute_schedule_state": 6, "alarm_item_state": 5, "alarm_ack_audit": 5}
	...
	expectedDefinitions := map[string][]string{
		...
		"compute_command_audit":  {"CHECK ((status = 'failed'::text) = (failure_code IS NOT NULL))", "CHECK (binding_epoch >= 1)", "CHECK (deployment_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'::text)", "CHECK (jsonb_typeof(result_event_ids) = 'array'::text)", "CHECK (length(idempotency_key) >= 16 AND length(idempotency_key) <= 128)", "CHECK (length(requested_by) >= 1 AND length(requested_by) <= 256)", "CHECK (result_version IS NULL OR result_version > 0)", "CHECK (status = ANY (ARRAY['queued'::text, 'running'::text, 'succeeded'::text, 'failed'::text]))", "PRIMARY KEY (deployment_id, command_id)", "UNIQUE (deployment_id, idempotency_key)"},
		...
		"alarm_ack_audit":        {"CHECK (deployment_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'::text)", "CHECK (length(comment) <= 2048)", "CHECK (length(subject_id) > 0)", "CHECK (state_version > 0)", "PRIMARY KEY (id)"},
	}
	...
	expectedIndexNames := []string{"schema_meta_pkey", "role_fence_pkey", "processed_event_pkey", "producer_fence_pkey", "consumer_checkpoint_pkey", "transactional_outbox_pkey", "transactional_outbox_deployment_id_dedupe_key_key", "transactional_outbox_claim_idx", "processing_failure_pkey", "processing_failure_deployment_id_dlq_id_key", "processing_failure_event_idx", "producer_sequence_pkey", "point_history_pkey", "point_history_order_idx", "point_current_pkey", "compute_input_snapshot_pkey", "compute_input_snapshot_event_idx", "compute_command_audit_pkey", "compute_command_audit_deployment_id_idempotency_key_key", "compute_command_audit_status_idx", "compute_trigger_state_pkey", "compute_schedule_state_pkey", "compute_schedule_due_idx", "alarm_item_state_pkey", "alarm_item_state_due_idx", "alarm_ack_audit_pkey", "alarm_ack_audit_lookup_idx"}
	...
	for table, fragment := range map[string]string{"transactional_outbox": "USING btree (deployment_id, state, next_attempt_at, lease_until, id)", "processing_failure": "USING btree (deployment_id, consumer_key, event_id, quarantined_at DESC) WHERE (event_id IS NOT NULL)", "point_history": "USING btree (deployment_id, point_id, epoch DESC, source_timestamp DESC, sequence DESC)", "compute_input_snapshot": "USING btree (deployment_id, compute_id, event_id)", "compute_command_audit": "USING btree (deployment_id, command_id, updated_at DESC)", "compute_schedule_state": "USING btree (deployment_id, next_run_at, compute_id)", "alarm_item_state": "USING btree (deployment_id, next_evaluation_at, alarm_item_id)"} {
