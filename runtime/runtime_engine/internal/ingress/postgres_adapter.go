...
func (a *PostgresAdapter) Process(ctx context.Context, message ValidatedMessage) (ProcessResult, error) {
	result, err := a.store.ProcessMessage(ctx, postgres.Message{DeploymentID: message.Event.DeploymentID, AccountID: message.Event.AccountID, ConsumerKey: message.Consumer.ConsumerKey, Role: message.Consumer.Role, Token: postgres.ConsumerRoleToken{OwnerID: message.Token.OwnerID, Epoch: message.Token.Epoch}, ProducerKey: message.ProducerKey, ProducerToken: postgres.ProducerToken{OwnerID: message.ProducerToken.OwnerID, Epoch: message.ProducerToken.Epoch}, EventID: message.Event.EventID, RawBody: message.RawBody, Subject: message.Event.Subject, CheckpointPosition: message.StreamPosition, DeliveryCount: message.DeliveryCount, OccurredAt: message.OccurredAt}, func(ctx context.Context, tx *postgres.BusinessTx) error {
		if a.handler == nil {
			return nil
		}
