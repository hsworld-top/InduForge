...
test('对象库 SDK 只使用 assetId，并生成受控内容地址', async () => {
  const runtime = createHttpRuntime({ baseUrl: '/api/v1/runtime', fetch: async (url) => ({ ok: true, json: async () => ({ code: 0, msg: 'ok', data: { items: [], assetId: String(url) } }) }) })
  const list = await runtime.assets.list({ page: 1 })
  assert.equal(list.code, 0)
  const item = await runtime.assets.get('asset/1')
  assert.equal(item.code, 0)
  const opened = await runtime.assets.open('asset/1')
  assert.equal(opened.data.assetId, 'asset/1')
  assert.match(opened.data.url, /assets\/asset%2F1\/content$/)
  assert.throws(() => runtime.assets.get(''), /assetId/)
})
