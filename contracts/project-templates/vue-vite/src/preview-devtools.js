...
function isTrustedMessageSource(source) {
  ...
  // Wujie 把 Designer 运行在宿主同源 iframe 中，内层 Preview 收到的 source
  // 是该 Wujie iframe，而不是 Preview 的直接 parent。
  try {
...
function setDrawerHeight(height) {
  ...
  devtoolsDrawer.style.height = `${clampDrawerHeight(height)}px`
}
...
function createDevtoolsDrawer() {
  ...
  resizeHandle.addEventListener('keydown', (event) => {
    ...
    setDrawerHeight(devtoolsDrawer.getBoundingClientRect().height + delta)
  })
...
async function togglePreviewDevtools() {
  ...
  erudaInstance.show('console')
}

export function installPreviewDevtoolsBridge() {
