...
public sealed class HslPanasonicMewtocolClient : IHslPanasonicMewtocolClient
{
    private readonly IReadWriteNet _client;
    private readonly PanasonicMewtocolOverTcp? _tcpClient;
    private readonly PanasonicMewtocol? _serialClient;
    private readonly SemaphoreSlim _operationGate = new(1, 1);
    private bool _connected;
    private bool _disposed;

    public HslPanasonicMewtocolClient(HslPanasonicMewtocolClientOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
        if (options.Transport == HslPanasonicMewtocolTransport.Tcp)
        {
            _tcpClient = new PanasonicMewtocolOverTcp(options.Host, options.Port, options.Station)
            {
                ConnectTimeOut = options.ConnectTimeoutMilliseconds,
                ReceiveTimeOut = options.ReceiveTimeoutMilliseconds,
            };
            _tcpClient.ByteTransform.DataFormat = MapDataFormat(options.DataFormat);
            _client = _tcpClient;
        }
        else
        {
            if (string.IsNullOrWhiteSpace(options.PortName))
                throw new ArgumentException("松下 Mewtocol 串口名称不能为空", nameof(options));
            _serialClient = new PanasonicMewtocol(options.Station) { ReceiveTimeOut = options.ReceiveTimeoutMilliseconds };
            _serialClient.SerialPortInni(options.PortName, options.BaudRate, options.DataBits, MapStopBits(options.StopBits), MapParity(options.Parity));
            _serialClient.ByteTransform.DataFormat = MapDataFormat(options.DataFormat);
            _client = _serialClient;
        }
    }
    ...
    public async Task<HslOperationResult> ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = _tcpClient is not null
                ? await _tcpClient.ConnectServerAsync().WaitAsync(cancellationToken).ConfigureAwait(false)
                : await Task.Run(_serialClient!.Open, cancellationToken).ConfigureAwait(false);
            _connected = result.IsSuccess;
            return result.IsSuccess
                ? HslOperationResult.Success()
                : HslOperationResult.Failure("HSL_PANASONIC_MEWTOCOL_CONNECT_FAILED", FormatError(result), retryable: true);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception)
        {
            _connected = false;
            return HslOperationResult.Failure("HSL_PANASONIC_MEWTOCOL_CONNECT_FAILED", exception.Message, retryable: true);
        }
    ...
    public async Task<HslOperationResult> CloseAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (_serialClient is not null)
            {
                await Task.Run(_serialClient.Close, cancellationToken).ConfigureAwait(false);
                _connected = false;
                return HslOperationResult.Success();
            }
            ...
            return result.IsSuccess
                ? HslOperationResult.Success()
                : HslOperationResult.Failure("HSL_PANASONIC_MEWTOCOL_CLOSE_FAILED", FormatError(result), retryable: false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception)
        {
            _connected = false;
            return HslOperationResult.Failure("HSL_PANASONIC_MEWTOCOL_CLOSE_FAILED", exception.Message, retryable: false);
        }
        finally { _operationGate.Release(); }
    }

    public async Task<HslReadResult> ReadAsync(HslPanasonicMewtocolReadRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            ThrowIfDisposed();
            if (!_connected) return HslReadResult.Failure("HSL_NOT_CONNECTED", "松下 Mewtocol 通信连接尚未建立", true);
            return await ReadCoreAsync(request, cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception)
        {
            return HslReadResult.Failure("HSL_PANASONIC_MEWTOCOL_READ_FAILED", exception.Message, true);
        }
        finally { _operationGate.Release(); }
    }
    ...
    private async Task<HslReadResult> ReadCoreAsync(HslPanasonicMewtocolReadRequest request, CancellationToken cancellationToken)
    {
        ...
        return request.DataType switch
        {
            ...
            _ => HslReadResult.Failure("HSL_DATA_TYPE_UNSUPPORTED", "松下 Mewtocol 数据类型不受支持", false),
        };
    }

    private async Task<HslReadResult> ReadRawAsync(
        HslPanasonicMewtocolReadRequest request,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return result.IsSuccess
            ...
            : HslReadResult.Failure("HSL_PANASONIC_MEWTOCOL_READ_FAILED", FormatError(result), true);
    }
    ...
    private static string FormatError(OperateResult result) => string.IsNullOrWhiteSpace(result.Message)
        ? $"松下 Mewtocol 操作失败，错误码 {result.ErrorCode}"
