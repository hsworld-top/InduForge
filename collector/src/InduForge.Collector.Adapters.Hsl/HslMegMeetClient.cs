...
public sealed class HslMegMeetClient : IHslMegMeetClient
{
    ...
    public HslMegMeetClient(HslMegMeetClientOptions options)
    {
    ...
    public async Task<HslOperationResult> ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = _device switch
            {
                ...
                _ => throw new InvalidOperationException("未知的麦格米特通信设备类型"),
            };
            _connected = result.IsSuccess;
            return result.IsSuccess ? HslOperationResult.Success() : OperationFailure("HSL_MEGMEET_CONNECT_FAILED", result, true);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_MEGMEET_CONNECT_FAILED", exception.Message, true); }
    ...
    public async Task<HslOperationResult> CloseAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else throw new InvalidOperationException("未知的麦格米特通信设备类型");
            _connected = false;
            return result.IsSuccess ? HslOperationResult.Success() : OperationFailure("HSL_MEGMEET_CLOSE_FAILED", result, false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_MEGMEET_CLOSE_FAILED", exception.Message, false); }
        finally { _operationGate.Release(); }
    }

    public async Task<HslReadResult> ReadAsync(HslMegMeetReadRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            ThrowIfDisposed();
            if (!_connected) return HslReadResult.Failure("HSL_NOT_CONNECTED", "麦格米特通信连接尚未建立", true);
            return await ReadCoreAsync(request, cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { return HslReadResult.Failure("HSL_MEGMEET_READ_FAILED", exception.Message, true); }
        finally { _operationGate.Release(); }
    }
    ...
    private static DeviceCommunication CreateDevice(HslMegMeetClientOptions options) => options.Transport switch
    {
        HslMegMeetTransport.Tcp => new MegMeetTcpNet(options.Host!, options.Port, options.Station) { ConnectTimeOut = options.ConnectTimeoutMilliseconds, ReceiveTimeOut = options.ReceiveTimeoutMilliseconds },
        HslMegMeetTransport.RtuOverTcp => new MegMeetSerialOverTcp(options.Host!, options.Port, options.Station) { ConnectTimeOut = options.ConnectTimeoutMilliseconds, ReceiveTimeOut = options.ReceiveTimeoutMilliseconds },
        HslMegMeetTransport.RtuSerial => CreateSerial(new MegMeetSerial(options.Station), options),
        _ => throw new ArgumentOutOfRangeException(nameof(options), options.Transport, "未知的麦格米特传输类型"),
    };

    private static DeviceSerialPort CreateSerial(DeviceSerialPort device, HslMegMeetClientOptions options)
    {
    ...
    private async Task<HslReadResult> ReadCoreAsync(HslMegMeetReadRequest request, CancellationToken cancellationToken)
    {
        ...
        return request.DataType switch
        {
            ...
            _ => HslReadResult.Failure("HSL_DATA_TYPE_UNSUPPORTED", "麦格米特读取数据类型不受支持", false),
        };
    }

    private async Task<HslReadResult> ReadRawAsync(HslMegMeetReadRequest request, Func<byte[], object> convert, CancellationToken cancellationToken)
    {
    ...
    private static HslReadResult ReadFailure(OperateResult result) => HslReadResult.Failure("HSL_MEGMEET_READ_FAILED", FormatError(result), true);
    ...
    private static string FormatError(OperateResult result) => string.IsNullOrWhiteSpace(result.Message) ? $"麦格米特通信操作失败，错误码 {result.ErrorCode}" : $"{result.Message}（通信错误码 {result.ErrorCode}）";
