...
public sealed class HslVigorClient : IHslVigorClient
{
    ...
    public HslVigorClient(HslVigorClientOptions options)
    {
    ...
    public async Task<HslOperationResult> ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = _device switch
            {
                ...
                _ => throw new InvalidOperationException("未知的丰炜通信设备类型"),
            };
            _connected = result.IsSuccess;
            return result.IsSuccess ? HslOperationResult.Success() : Failure("HSL_VIGOR_CONNECT_FAILED", result, true);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_VIGOR_CONNECT_FAILED", exception.Message, true); }
    ...
    public async Task<HslOperationResult> CloseAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else throw new InvalidOperationException("未知的丰炜通信设备类型");
            _connected = false;
            return result.IsSuccess ? HslOperationResult.Success() : Failure("HSL_VIGOR_CLOSE_FAILED", result, false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_VIGOR_CLOSE_FAILED", exception.Message, false); }
        finally { _operationGate.Release(); }
    }

    public async Task<HslReadResult> ReadAsync(HslVigorReadRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            ThrowIfDisposed();
            if (!_connected) return HslReadResult.Failure("HSL_NOT_CONNECTED", "丰炜通信连接尚未建立", true);
            return await ReadCoreAsync(request, cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { return HslReadResult.Failure("HSL_VIGOR_READ_FAILED", exception.Message, true); }
        finally { _operationGate.Release(); }
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        _disposed = true; _device.Dispose(); _operationGate.Dispose();
    }

    private static DeviceCommunication CreateDevice(HslVigorClientOptions options)
    {
        if (options.Transport == HslVigorTransport.SerialOverTcp)
            return new VigorSerialOverTcp(options.Host!, options.Port) { Station = options.Station, ConnectTimeOut = options.ConnectTimeoutMilliseconds, ReceiveTimeOut = options.ReceiveTimeoutMilliseconds };
        var serial = new VigorSerial { Station = options.Station };
        ...
        serial.ReceiveTimeOut = options.ReceiveTimeoutMilliseconds;
        return serial;
    }

    private async Task<HslReadResult> ReadCoreAsync(HslVigorReadRequest request, CancellationToken cancellationToken)
    {
        ...
        return request.DataType switch
        {
            ...
            _ => HslReadResult.Failure("HSL_DATA_TYPE_UNSUPPORTED", "丰炜读取数据类型不受支持", false),
        };
    }

    private async Task<HslReadResult> ReadRawAsync(HslVigorReadRequest request, Func<byte[], object> convert, CancellationToken cancellationToken)
    {
    ...
    private static HslReadResult ReadFailure(OperateResult result) => HslReadResult.Failure("HSL_VIGOR_READ_FAILED", Format(result), true);
    private static HslOperationResult Failure(string code, OperateResult result, bool retryable) => HslOperationResult.Failure(code, Format(result), retryable);
    private static string Format(OperateResult result) => string.IsNullOrWhiteSpace(result.Message) ? $"丰炜通信操作失败，错误码 {result.ErrorCode}" : $"{result.Message}（通信错误码 {result.ErrorCode}）";
