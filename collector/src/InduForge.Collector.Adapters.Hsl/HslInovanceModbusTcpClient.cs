...
public sealed class HslInovanceModbusTcpClient : IHslInovanceModbusTcpClient
{
    ...
    public HslInovanceModbusTcpClient(HslInovanceModbusTcpClientOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
        _client = new InovanceTcpNet(MapSeries(options.Series), options.Host, options.Port, options.Station)
        {
        ...
        _client.ByteTransform.IsStringReverseByteWord = options.StringReverse;
    }
    ...
    public async Task<HslOperationResult> ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _connected = false;
            return HslOperationResult.Failure("HSL_INOVANCE_MODBUS_TCP_CONNECT_FAILED", exception.Message, true);
        }
    ...
    public async Task<HslOperationResult> CloseAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _connected = false;
            return HslOperationResult.Failure("HSL_INOVANCE_MODBUS_TCP_CLOSE_FAILED", exception.Message, false);
        }
        finally { _operationGate.Release(); }
    }

    public async Task<HslReadResult> ReadAsync(HslInovanceModbusTcpReadRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            ThrowIfDisposed();
            if (!_connected) return HslReadResult.Failure("HSL_NOT_CONNECTED", "汇川 Modbus TCP 通信连接尚未建立", true);
            return await ReadCoreAsync(request, cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception)
        {
            return HslReadResult.Failure("HSL_INOVANCE_MODBUS_TCP_READ_FAILED", exception.Message, true);
        }
        finally { _operationGate.Release(); }
    }
    ...
    private async Task<HslReadResult> ReadCoreAsync(HslInovanceModbusTcpReadRequest request, CancellationToken cancellationToken)
    {
        ...
        return request.DataType switch
        {
            ...
            _ => HslReadResult.Failure("HSL_DATA_TYPE_UNSUPPORTED", "汇川 Modbus TCP 数据类型不受支持", false),
        };
    }

    private async Task<HslReadResult> ReadRawAsync(
        HslInovanceModbusTcpReadRequest request,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return result.IsSuccess
            ...
            : HslReadResult.Failure("HSL_INOVANCE_MODBUS_TCP_READ_FAILED", FormatError(result), true);
    }
    ...
    private static HslOperationResult Failure(string operation, OperateResult result, bool retryable) =>
        HslOperationResult.Failure($"HSL_INOVANCE_MODBUS_TCP_{operation}_FAILED", FormatError(result), retryable);

    private static InovanceSeries MapSeries(HslInovanceSeries series) => series switch
    {
        HslInovanceSeries.AM => InovanceSeries.AM,
        HslInovanceSeries.H3U => InovanceSeries.H3U,
        HslInovanceSeries.H5U => InovanceSeries.H5U,
        HslInovanceSeries.Easy => InovanceSeries.Easy,
    ...
    private static string FormatError(OperateResult result) => string.IsNullOrWhiteSpace(result.Message)
        ? $"汇川 Modbus TCP 操作失败，错误码 {result.ErrorCode}"
