...
public sealed class HslFujiClient : IHslFujiClient
{
    ...
    public HslFujiClient(HslFujiClientOptions options)
    {
    ...
    public async Task<HslOperationResult> ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = _device switch
            {
                ...
                _ => throw new InvalidOperationException("未知的富士通信设备类型"),
            };
            _connected = result.IsSuccess;
            return result.IsSuccess ? HslOperationResult.Success() : OperationFailure("HSL_FUJI_CONNECT_FAILED", result, true);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_FUJI_CONNECT_FAILED", exception.Message, true); }
    ...
    public async Task<HslOperationResult> CloseAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else throw new InvalidOperationException("未知的富士通信设备类型");
            _connected = false;
            return result.IsSuccess ? HslOperationResult.Success() : OperationFailure("HSL_FUJI_CLOSE_FAILED", result, false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_FUJI_CLOSE_FAILED", exception.Message, false); }
        finally { _operationGate.Release(); }
    }

    public async Task<HslReadResult> ReadAsync(HslFujiReadRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            ThrowIfDisposed();
            if (!_connected) return HslReadResult.Failure("HSL_NOT_CONNECTED", "富士通信连接尚未建立", true);
            return await ReadCoreAsync(request, cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { return HslReadResult.Failure("HSL_FUJI_READ_FAILED", exception.Message, true); }
        finally { _operationGate.Release(); }
    }
    ...
    {
        if (_disposed) return;
        if (_connected) await CloseAsync(CancellationToken.None).ConfigureAwait(false);
        _disposed = true;
        _device.Dispose();
        _operationGate.Dispose();
    }

    private static DeviceCommunication CreateDevice(HslFujiClientOptions options)
    {
        return options.Protocol switch
        {
            HslFujiProtocol.CommandSettingTypeTcp => new FujiCommandSettingType(options.Host!, options.Port)
            {
                DataSwap = options.DataSwap,
                ConnectTimeOut = options.ConnectTimeoutMilliseconds,
                ReceiveTimeOut = options.ReceiveTimeoutMilliseconds,
            },
            HslFujiProtocol.SphTcp => new FujiSPHNet(options.Host!, options.Port)
            {
                ConnectionID = options.ConnectionId,
                ConnectTimeOut = options.ConnectTimeoutMilliseconds,
                ReceiveTimeOut = options.ReceiveTimeoutMilliseconds,
            },
            HslFujiProtocol.SpbOverTcp => new FujiSPBOverTcp(options.Host!, options.Port)
            {
                Station = options.Station,
                ConnectTimeOut = options.ConnectTimeoutMilliseconds,
                ReceiveTimeOut = options.ReceiveTimeoutMilliseconds,
            },
            HslFujiProtocol.SpbSerial => CreateSerial(new FujiSPB { Station = options.Station }, options),
            _ => throw new ArgumentOutOfRangeException(nameof(options)),
        };
    }

    private static DeviceSerialPort CreateSerial(DeviceSerialPort device, HslFujiClientOptions options)
    {
    ...
    private async Task<HslReadResult> ReadCoreAsync(HslFujiReadRequest request, CancellationToken cancellationToken)
    {
        ...
        return request.DataType switch
        {
            ...
            _ => HslReadResult.Failure("HSL_DATA_TYPE_UNSUPPORTED", "富士读取数据类型不受支持", false),
        };
    }

    private async Task<HslReadResult> ReadRawAsync(HslFujiReadRequest request, Func<byte[], object> convert, CancellationToken cancellationToken)
    {
    ...
    private static HslReadResult ReadFailure(OperateResult result) => HslReadResult.Failure("HSL_FUJI_READ_FAILED", FormatError(result), true);
    ...
    private static string FormatError(OperateResult result) => string.IsNullOrWhiteSpace(result.Message) ? $"富士通信操作失败，错误码 {result.ErrorCode}" : $"{result.Message}（通信错误码 {result.ErrorCode}）";
