...
public sealed class HslXinjeClient : IHslXinjeClient
{
    ...
    public HslXinjeClient(HslXinjeClientOptions options)
    {
    ...
    public async Task<HslOperationResult> ConnectAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = _device switch
            {
                ...
                _ => throw new InvalidOperationException("未知的信捷通信设备类型"),
            };
            _connected = result.IsSuccess;
            return result.IsSuccess ? HslOperationResult.Success() : OperationFailure("HSL_XINJE_CONNECT_FAILED", result, true);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_XINJE_CONNECT_FAILED", exception.Message, true); }
    ...
    public async Task<HslOperationResult> CloseAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else throw new InvalidOperationException("未知的信捷通信设备类型");
            _connected = false;
            return result.IsSuccess ? HslOperationResult.Success() : OperationFailure("HSL_XINJE_CLOSE_FAILED", result, false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { _connected = false; return HslOperationResult.Failure("HSL_XINJE_CLOSE_FAILED", exception.Message, false); }
        finally { _operationGate.Release(); }
    }

    public async Task<HslReadResult> ReadAsync(HslXinjeReadRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            ThrowIfDisposed();
            if (!_connected) return HslReadResult.Failure("HSL_NOT_CONNECTED", "信捷通信连接尚未建立", true);
            return await ReadCoreAsync(request, cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception exception) { return HslReadResult.Failure("HSL_XINJE_READ_FAILED", exception.Message, true); }
        finally { _operationGate.Release(); }
    }
    ...
    private static DeviceCommunication CreateDevice(HslXinjeClientOptions options)
    {
        var series = options.Series switch { HslXinjeSeries.XD => XinJESeries.XD, HslXinjeSeries.XL => XinJESeries.XL, _ => XinJESeries.XC };
        return options.Transport switch
        {
            HslXinjeTransport.Tcp => new XinJETcpNet(series, options.Host!, options.Port, options.Station) { ConnectTimeOut = options.ConnectTimeoutMilliseconds, ReceiveTimeOut = options.ReceiveTimeoutMilliseconds },
            HslXinjeTransport.RtuOverTcp => new XinJESerialOverTcp(series, options.Host!, options.Port, options.Station) { ConnectTimeOut = options.ConnectTimeoutMilliseconds, ReceiveTimeOut = options.ReceiveTimeoutMilliseconds },
            HslXinjeTransport.RtuSerial => CreateSerial(new XinJESerial(series, options.Station), options),
            HslXinjeTransport.InternalTcp => new XinJEInternalNet(options.Host!, options.Port, options.Station) { ConnectTimeOut = options.ConnectTimeoutMilliseconds, ReceiveTimeOut = options.ReceiveTimeoutMilliseconds },
            _ => throw new ArgumentOutOfRangeException(nameof(options), options.Transport, "未知的信捷传输类型"),
        };
    }

    private static DeviceSerialPort CreateSerial(DeviceSerialPort device, HslXinjeClientOptions options)
    {
    ...
    private async Task<HslReadResult> ReadCoreAsync(HslXinjeReadRequest request, CancellationToken cancellationToken)
    {
        ...
        return request.DataType switch
        {
            ...
            _ => HslReadResult.Failure("HSL_DATA_TYPE_UNSUPPORTED", "信捷读取数据类型不受支持", false),
        };
    }

    private async Task<HslReadResult> ReadRawAsync(HslXinjeReadRequest request, Func<byte[], object> convert, CancellationToken cancellationToken)
    {
    ...
    private static HslReadResult ReadFailure(OperateResult result) => HslReadResult.Failure("HSL_XINJE_READ_FAILED", FormatError(result), true);
    private static HslOperationResult OperationFailure(string code, OperateResult result, bool retryable) => HslOperationResult.Failure(code, FormatError(result), retryable);
    private static string FormatError(OperateResult result) => string.IsNullOrWhiteSpace(result.Message) ? $"信捷通信操作失败，错误码 {result.ErrorCode}" : $"{result.Message}（通信错误码 {result.ErrorCode}）";
