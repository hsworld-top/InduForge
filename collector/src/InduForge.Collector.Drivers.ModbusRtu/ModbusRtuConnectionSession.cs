...
internal sealed class ModbusRtuConnectionSession : IIndustrialConnectionSession, IPointReaderSession
{
    private static readonly IReadOnlyList<DriverDiagnostic> NoDiagnostics = [];
    private readonly IHslModbusRtuClient _client;

    public ModbusRtuConnectionSession(IHslModbusRtuClient client, string serverName)
    {
        _client = client;
    ...
    public async Task<ReadResult> ReadAsync(ReadRequest request, CancellationToken cancellationToken)
    {
        ...
        for (var index = 0; index < request.Points.Count; index++)
        {
            ...
            try
            {
                validPoints.Add(new ModbusReadPoint(index, point, ModbusRtuAddress.Parse(point)));
            }
            catch (ModbusRtuDriverException exception)
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (segment.Points.Count == 1)
        {
            ...
            values[point.Index] = Failure(
                ...
                result.ErrorMessage ?? "Modbus RTU 变量读取失败");
            return;
        }
        ...
        foreach (var point in segment.Points)
        {
            ...
            values[point.Index] = singleResult.Succeeded
                ...
                : Failure(
                    ...
                    singleResult.ErrorMessage ?? "Modbus RTU 变量读取失败");
        }
    }
    ...
    private static bool FitsProtocolReadLimit(ModbusReadPoint first, ModbusReadPoint last)
    {
        ...
        if (first.Address.ValueType == HslValueType.Boolean &&
            first.Address.Area is ModbusRtuArea.InputRegister or ModbusRtuArea.HoldingRegister)
        {
            var firstRegister = start / 16;
            var endRegister = (end + 15) / 16;
            return endRegister - firstRegister <= 125;
        }
        return end - start <= (first.Address.Area is ModbusRtuArea.Coil or ModbusRtuArea.DiscreteInput ? 2000 : 125);
    }
    ...
    private static long GetStartUnit(ModbusReadPoint point)
    {
        if (point.Address.ValueType == HslValueType.Boolean &&
            point.Address.Area is ModbusRtuArea.InputRegister or ModbusRtuArea.HoldingRegister)
        {
    ...
    private sealed record ModbusReadPoint(int Index, PointReadRequest Request, ModbusRtuAddress Address);
