...
class Graph {
  ...
  isConnected(x = this.adj[0].data) {
    ...
    for (let i = 0; i < len; i++) {
      if (!visited[i]) {
        unreachableNode.push(this.adj[i])
      };
    }
    console.log('不可达节点数组为:');
    console.log(unreachableNode);
  ...
  getShortestPath(x, y) {
    ...
    var result = '';
    result = path.find(item => {
      return item.split('->')[item.split('->').length - 1] === y
    })
    return result
  }
}
