...
ht.Default.def('GaoluDialog', ht.widget.Dialog, {
    ok: function () {
        var self = this;
        var url = self.formPane.v('url');
        if (!url) return;
        var params = {
            path: url,
            content: ht.Default.stringify(self.json)
        };
        self.editor.request('upload', params, function (data) {
            params = {
                path: url.substr(0, url.length - 5) + '.png',
                content: self.g3d.toDataURL(self.dm.getBackground())
            };
            self.editor.request('upload', params, function (data) {
                self.hide();
            });
