...
import { getInduForgeWorkspaceRoot, isInduForgeWorkspaceRoot } from "./induforge-config";
...
async function loadAllSessions(): Promise<SessionInfo[]> {
  ...
  const workspaceSessions = piSessions.filter((session) => isInduForgeWorkspaceRoot(session.cwd));
  const pathToId = new Map<string, string>();
  for (const s of workspaceSessions) pathToId.set(sessionPathKey(s.path), s.id);

  const sessions = workspaceSessions.map((s) => {
  ...
  return sessions.map((session) => ({
    ...session,
    projectRoot: getInduForgeWorkspaceRoot(),
  }));
}
