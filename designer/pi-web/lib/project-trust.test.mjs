...
test("all project resource loaders and reloads enforce project trust", async () => {
  ...
  assert.match(skillsSource, /additionalSkillPaths: \[getProjectSkillsDir\(\)\]/);
  assert.match(skillsSource, /await loader\.reload\(\)/);
  assert.match(skillsInstallSource, /scope !== "project"/);
  assert.match(skillsInstallSource, /isInduForgeWorkspaceRoot\(cwd\)/);
  assert.match(pluginsSource, /Plugin management is disabled/);
  assert.doesNotMatch(pluginsSource, /DefaultResourceLoader|projectTrusted|scope === "project"/);
});
