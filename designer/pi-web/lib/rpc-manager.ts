...
import { getInduForgeResourceLoaderOptions } from "./induforge-agent-resources";
import { isInduForgeWorkspaceRoot } from "./induforge-config";
...
export async function startRpcSession(
  ...
  if (!isInduForgeWorkspaceRoot(sessionCwd)) {
    throw new Error("Session workspace is outside the configured workspace");
  }
  ...
  const starting = (async () => {
    ...
    const services = await createAgentSessionServices({
      cwd: sessionCwd,
      agentDir,
      resourceLoaderOptions: getInduForgeResourceLoaderOptions(),
