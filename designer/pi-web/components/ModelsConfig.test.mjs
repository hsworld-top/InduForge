...
test("custom model config exposes model headers without a system-prompt compatibility toggle", () => {
  ...
  assert.doesNotMatch(source, /effectiveCompat\(provider, model\)\["supportsDeveloperRole"\] !== false/);
  assert.doesNotMatch(source, /setCompatBool\(model, "supportsDeveloperRole", v\)/);
});
