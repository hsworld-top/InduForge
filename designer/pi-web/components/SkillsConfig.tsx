...
function AddSkillPanel({
    ...
    async (pkg: string) => {
      ...
      try {
        const res = await fetch("/api/skills/install", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ package: pkg, scope: "project", cwd }),
        });
  ...
  const installPath = `${shortenPath(cwd)}/.pi/skills/`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        ...
        {/* InduForge 嵌入版只允许将技能安装到当前工程。 */}
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 12, color: "var(--text-muted)", fontWeight: 600 }}>project</span>
          <span
            style={{
              fontSize: 12,
              color: "var(--text-dim)",
      ...
      {results.length > 0 ? (
        <div style={{ flex: 1, overflowY: "auto" }}>
          {results.map((r) => {
            const isInstalled =
              installedPackages.has(r.package) || newlyInstalledPkgs.has(r.package);
