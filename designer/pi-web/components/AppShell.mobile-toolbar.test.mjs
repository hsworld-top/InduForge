...
const source = (await readFile(new URL("./AppShell.tsx", import.meta.url), "utf8")).replaceAll("\r\n", "\n");

test("uses a compact mobile toolbar with only embedded workspace actions", () => {
  ...
  for (const removedAction of ["history", "system", "theme", "language"]) {
...
test("keeps the mobile action layer open after using the remaining actions", () => {
  ...
  assert.doesNotMatch(source, /renderThemeButton|renderLanguageButton|handleViewFullHistory|handleSystemPromptToggle/);
});
