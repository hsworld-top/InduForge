...
export function SessionSidebar({ fixedWorkspaceRoot, selectedSessionId, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, onBackgroundTaskDone, onRunningSessionIdsChange }: Props) {
  ...
  useEffect(() => {
    if (fixedWorkspaceRoot) setSelectedCwd(fixedWorkspaceRoot);
  }, [fixedWorkspaceRoot]);
  ...
  }, [fixedWorkspaceRoot, selectedCwd, wtRefreshKey, refreshKey]);
  ...
  const selectedProject = fixedWorkspaceRoot ?? projectRootFor(selectedCwd);
  ...
  const filteredSessions = fixedWorkspaceRoot
    ? allSessions.filter((session) => session.cwd === fixedWorkspaceRoot)
    : selectedProject
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        {/* CWD picker */}
        {!fixedWorkspaceRoot && <div ref={dropdownRef} style={{ position: "relative" }}>
          <button
        ...
        {!fixedWorkspaceRoot && showWorktreeSwitcher && (() => {
        ...
        {!fixedWorkspaceRoot && inactiveWorktreeSelector && (
          <button
            type="button"
            aria-disabled="true"
            tabIndex={-1}
