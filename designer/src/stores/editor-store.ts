...
export const useEditorStore = defineStore('editor', () => {
  ...
  const syncElLayoutRows = (layoutId: string, nextProps: Record<string, unknown>) => {
    ...
    refreshedRows.forEach((rowId) => {
      ...
      const nextColumns = Math.max(1, Math.min(24, Number(rowNode.props?.columns || colCount || 3)))
  ...
  const reorderNodeLayer = (
    ...
    const siblings = parent.children
      ...
      .filter((item): item is { id: string; sourceIndex: number; zIndex: number } => item !== null)
      .sort((a, b) => {
