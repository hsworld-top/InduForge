// @ts-nocheck
// 遗留 v1 schema 形状松散；待迁移管线收紧后再逐步补类型
/**
 * Schema 版本迁移
 * 支持从旧版本 Schema 迁移到新版本
 */

import { CURRENT_SCHEMA_VERSION, generateId } from "./types.ts";
...
const migrations = {
  ...
  1: (schema) => {
    ...
    if (schema.projectVariables && typeof schema.projectVariables === "object") {
      Object.assign(v2Schema.vars.global, migrateVariables(schema.projectVariables));
    }
    ...
    if (schema.components && Array.isArray(schema.components)) {
      ...
      for (const component of schema.components) {
        ...
        v2Schema.nodesById[rootNodeId].children.push(component.id || generateId("node_"));
      }
    }
...
function migrateLayoutItem(component) {
  ...
  // 检查是否有绝对定位
  if (style.position === "absolute" || style.left !== undefined || style.top !== undefined) {
    return {
      free: {
        mode: "abs",
        abs: {
          ...
          w: Number.parseFloat(style.width) || 100,
          h: Number.parseFloat(style.height) || 100,
          z: style.zIndex || 0,
        },
      },
    };
