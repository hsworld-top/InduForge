...
export function useCanvasViewportPlacement(opts: UseCanvasViewportPlacementOptions): void {
  ...
  function keepViewportPointStableOnZoom(
    anchor: CanvasViewportZoomAnchor,
    nextZoom: number,
  ): void {
  ...
  function keepViewportCenterStableOnZoom(prevZoom: number, nextZoom: number) {
    ...
    const canvasX =
      (centerX - opts.rulerInset.value - opts.translateX.value - prevOverflowOffset.left) /
      prevZoom;
    const canvasY =
      (centerY - opts.rulerInset.value - opts.translateY.value - prevOverflowOffset.top) /
      prevZoom;
