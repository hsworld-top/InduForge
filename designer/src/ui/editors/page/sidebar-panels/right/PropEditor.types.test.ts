...
describe('PropEditor type mapping', () => {
    ...
    mount(PropEditor, {
      ...
      global: {
        stubs: {
          ...
          'el-input': componentStub(
            'ElInput',
            '<textarea v-if="type === \'textarea\'" /><input v-else />',
          ),
  ...
  it('renders boolean, enum, number and JSON props with matched controls', () => {
    expect(
      mountEditor({ type: 'boolean' }, true).findComponent({ name: 'ElSwitch' }).exists(),
    ).toBe(true)
    ...
    expect(
      mountEditor({ type: 'number', min: 1 }, 1).findComponent({ name: 'ElInputNumber' }).exists(),
    ).toBe(true)
