...
function createDslFactory(type: string) {
  return (config: AnyRecord = {}) => ({
    ...
    bindings: config?.bindings && typeof config.bindings === 'object' ? config.bindings : undefined,
    conditions:
  ...
  const presets: AnyArray = [
    {
      ...
      label: '鼠标移入选中',
      content: buildDslTemplate(methodName, '  hoverSelect: true,'),
    },
  ]
...
const currentPresetOptions = computed<AnyArray>(() => {
  ...
  if (type === 'Button') {
    return withCommonStylePresets(buttonStylePresets.map(withButtonRootSelectorSupport)).map(
      (preset: AnyRecord) => ({
        ...localizePropertyPanelPreset(preset),
  ...
  if (customStylePresetMap[type]) {
    return withCommonStylePresets(customStylePresetMap[type] as AnyArray).map(
      (preset: AnyRecord) => ({
        ...localizePropertyPanelPreset(preset),
        tags: preset.tags || [type, 'style', 'custom'],
        keywords: preset.keywords || [preset.label, type, 'custom'].filter(Boolean).join(' '),
...
const isCollapseNode = computed<boolean>(
  () => elementType.value === 'Collapse' && !!selectedNode.value,
)
...
const currentPositionStyle = computed<AnyRecord>(() => {
  ...
  if (node) {
    ...
    if (absolutePos && (Number.isFinite(absolutePos.x) || Number.isFinite(absolutePos.y))) {
      return {
        ...style,
          ...
          <div v-if="isElContainer" class="prop-section region-config-section">
            ...
            <div class="prop-section-body region-config-body">
              ...
              <div v-if="visibleRegionSizeRows.length" class="region-config-block">
                ...
                <div v-for="item in visibleRegionSizeRows" :key="item.key" class="region-size-row">
