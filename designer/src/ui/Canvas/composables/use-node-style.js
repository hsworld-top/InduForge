...
import { getDescriptor, isContainerType } from "@/components/registry.js";
import { resolveDescriptorContainerStyle } from "@/components/registry.js";
import { componentRegistry } from "@/editor-core";
import {
  normalizeStyleObject,
  resolveTextPropStyle,
} from "@/editor-core/utils/styleUtils.js";
  ...
  function resolveLayoutStyle(currentNode, isRoot) {
    ...
    if (currentNode.layoutItem?.free) {
      ...
      if (free.mode === "constraints" && free.constraints) {
        const c = free.constraints;
        style.position = "absolute";
        if (c.left !== undefined) style.left = `${c.left}px`;
        if (c.right !== undefined) style.right = `${c.right}px`;
        if (c.top !== undefined) style.top = `${c.top}px`;
        if (c.bottom !== undefined) style.bottom = `${c.bottom}px`;
        if (c.width !== undefined) style.width = `${c.width}px`;
        if (c.height !== undefined) style.height = `${c.height}px`;
      }
    }
