...
const sharedStubs = {
  ...
  'el-dropdown': {
    props: ['hideOnClick'],
    template:
      '<div class="dropdown-stub" :data-hide-on-click="String(hideOnClick)"><slot /><slot name="dropdown" /></div>',
  },
  'el-dropdown-menu': {
...
describe('TopToolbar i18n', () => {
  ...
  it('会随 locale 切换更新关键按钮文案', async () => {
    const wrapper = mount(TopToolbar, {
      ...
      props: {
        viewPresets: [{ key: 'pc', label: 'PC', width: 1366, height: 768 }],
        saveSettings: {
          autoSave: true,
          intervalMinutes: 5,
        },
  ...
  it('预览身份只展示登录账号，并保持内层选择器不传送', () => {
    ...
    expect(
      wrapper
        ...
        .some((item) => item.attributes('data-hide-on-click') === 'false'),
    ).toBe(true)
    expect(
