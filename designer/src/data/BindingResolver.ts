...
export class BindingResolver extends EventEmitter {
  ...
  private _resolveDatapoint(binding: any, _context: Record<string, unknown>): any {
    ...
    if (this._mode === 'edit') {
      const mockValue = this._mockProvider ? this._mockProvider.getValue(path, binding) : designMock
      const value = isValidValue(mockValue) ? applyTransforms(mockValue, transform || []) : fallback
