 ...
 * @typedef {import('./types.js').BindingResolverOptions} BindingResolverOptions
 * @typedef {import('./types.js').ResolvedBinding} ResolvedBinding
 * @typedef {import('./types.js').DatapointStatus} DatapointStatus
 * @typedef {import('./types.js').ExpressionContext} ExpressionContext
 * @typedef {import('../editor-core/types.js').Binding} Binding
 */
...
export class BindingResolver extends EventEmitter {
   ...
   * @param {import('./DataService.js').DataService} [dependencies.dataService] - 数据服务
   * @param {import('./MockDataProvider.js').MockDataProvider} [dependencies.mockProvider] - Mock 提供者
   * @param {import('./VarsStore.js').VarsStore} [dependencies.varsStore] - 变量存储
   * @param {import('./ExpressionEngine.js').ExpressionEngine} [dependencies.expressionEngine] - 表达式引擎
   * @param {import('./DiagnosticsStore.js').DiagnosticsStore} [dependencies.diagnosticsStore] - 诊断存储
   * @param {BindingResolverOptions} [options] - 配置选项
   */
  constructor(dependencies = {}, options = {}) {
    super();

    /** @type {import('./DataService.js').DataService | null} */
    this._dataService = dependencies.dataService || null;

    /** @type {import('./MockDataProvider.js').MockDataProvider | null} */
    this._mockProvider = dependencies.mockProvider || null;

    /** @type {import('./VarsStore.js').VarsStore | null} */
    this._varsStore = dependencies.varsStore || null;

    /** @type {import('./ExpressionEngine.js').ExpressionEngine | null} */
    this._expressionEngine = dependencies.expressionEngine || null;

    /** @type {import('./DiagnosticsStore.js').DiagnosticsStore | null} */
    ...
    /** @type {Map<string, Set<function>>} 绑定值更新回调 */
    this._bindingCallbacks = new Map();

    /** @type {Map<string, function>} 取消订阅函数 */
  ...
  _subscribeExpr(bindingId, binding, callback, context) {
    ...
    const varChangeHandler = (data) => {
      const isPageVar =
        data.scope === "page" && varDeps.page.includes(data.name);
      const isGlobalVar =
        data.scope === "global" && varDeps.global.includes(data.name);
