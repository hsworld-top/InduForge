 ...
 * @typedef {import('./types.js').SubscriptionOptions} SubscriptionOptions
...
export class DataService extends EventEmitter {
  /**
   * 创建数据服务
   * @param {DataServiceOptions} [options] - 配置选项
   */
  constructor(options = {}) {
    super();
    ...
    /** @type {Map<string, *>} 数据点值缓存 */
    this._valueCache = new Map();

    /** @type {Map<string, Set<function>>} 订阅回调 */
    this._subscriptions = new Map();

    /** @type {Set<string>} 已订阅到服务器的路径 */
   ...
   * @param {SubscriptionOptions} [options] - 选项
  ...
  /**
   * 处理值更新
   * @param {DatapointValue} data - 值数据
   * @private
   */
  _handleValueUpdate(data) {
    ...
    const callbacks = this._subscriptions.get(path);
    if (callbacks) {
      const payload = { path, value, timestamp };
