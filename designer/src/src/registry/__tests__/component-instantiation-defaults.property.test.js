...
import { registerComponent, getComponent, createComponentInstance, clearRegistry, getAllComponents } from '../index.js';
...
describe('Property 16: Component Instantiation Defaults', () => {
    ...
    it('should apply all defaultProps when creating a component instance', () => {
        ...
        fc.assert(
            fc.property(fc.constantFrom(...basicComponents.map((c) => c.type)), (componentType) => {
    ...
    it('should apply arbitrary defaultProps from any component definition', () => {
        ...
        const propValueArb = fc.oneof(fc.string({ minLength: 1, maxLength: 20 }), fc.integer({ min: 0, max: 1000 }), fc.boolean(), fc.constant(null));

        const defaultPropsArb = fc.dictionary(fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9]{0,9}$/), propValueArb, { minKeys: 1, maxKeys: 5 });

        const componentTypeArb = fc.stringMatching(/^[A-Z][a-zA-Z]{2,15}$/);

        fc.assert(
            fc.property(componentTypeArb, defaultPropsArb, (type, defaultProps) => {
                clearRegistry();
    ...
    it('should apply arbitrary defaultStyle from any component definition', () => {
        const styleValueArb = fc.oneof(fc.integer({ min: 0, max: 1000 }), fc.constantFrom('absolute', 'relative', 'fixed'), fc.stringMatching(/^#[0-9a-fA-F]{6}$/));

        const defaultStyleArb = fc.dictionary(fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9]{0,9}$/), styleValueArb, { minKeys: 1, maxKeys: 5 });

        const componentTypeArb = fc.stringMatching(/^[A-Z][a-zA-Z]{2,15}$/);

        fc.assert(
            fc.property(componentTypeArb, defaultStyleArb, (type, defaultStyle) => {
                clearRegistry();
    ...
    it('should merge overrides with defaults, preserving non-overridden values', () => {
        ...
        fc.assert(
            fc.property(
                ...
                fc.record({
                    props: fc.dictionary(fc.constantFrom('customProp1', 'customProp2'), fc.string({ minLength: 1, maxLength: 10 }), { minKeys: 0, maxKeys: 2 }),
                    style: fc.record({
    ...
    it('should return null for unregistered component types', () => {
        fc.assert(
            fc.property(fc.stringMatching(/^[A-Z][a-zA-Z]{5,15}$/), (unknownType) => {
                clearRegistry();
    ...
    it('should generate unique IDs for each instance', () => {
        ...
        fc.assert(
            fc.property(fc.constantFrom(...basicComponents.map((c) => c.type)), fc.integer({ min: 2, max: 10 }), (componentType, count) => {
                const ids = new Set();
