...
describe('Property 9: Locked Component Protection', () => {
    ...
    it('should prevent selection of locked components when clicked', () => {
        fc.assert(
            fc.property(lockedComponentArb, fc.array(unlockedComponentArb, { minLength: 0, maxLength: 3 }), (lockedComponent, otherComponents) => {
    ...
    it('should prevent dragging of locked components', () => {
        fc.assert(
            fc.property(lockedComponentArb, (lockedComponent) => {
                const pinia = createPinia();
                setActivePinia(pinia);
                const store = useDesignStore();
    ...
    it('should allow selection of unlocked components when clicked', () => {
        fc.assert(
            fc.property(unlockedComponentArb, (unlockedComponent) => {
                const pinia = createPinia();
                setActivePinia(pinia);
                const store = useDesignStore();
    ...
    it('should apply locked styling to locked components', () => {
        fc.assert(
            fc.property(lockedContainerWithChildrenArb, (lockedContainer) => {
                const pinia = createPinia();
                setActivePinia(pinia);
                const store = useDesignStore();
