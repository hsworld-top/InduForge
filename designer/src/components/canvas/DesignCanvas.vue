  ...
  if (line) {
    ...
    if (isVertical) {
      ...
      if (Number.isFinite(event?.clientY)) {
        const minY = line.y;
        const maxY = Math.max(line.y, line.y + line.height - height);
        y = clampValue(event.clientY - height / 2, minY, maxY);
      } else {
      ...
      if (Number.isFinite(event?.clientX)) {
        const minX = line.x;
        const maxX = Math.max(line.x, line.x + line.width - width);
        x = clampValue(event.clientX - width / 2, minX, maxX);
      } else {
