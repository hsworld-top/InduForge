...
const markerContainsPosition = (marker, position) => {
    ...
    if (position.lineNumber < marker.startLineNumber || position.lineNumber > marker.endLineNumber) return false;
    if (position.lineNumber === marker.startLineNumber && position.column < marker.startColumn) return false;
    if (position.lineNumber === marker.endLineNumber && position.column > marker.endColumn) return false;
    return true;
};
...
defineExpose({
    ...
    insertText: (text) => {
        ...
        const range = selection || new monaco.Range(position.lineNumber, position.column, position.lineNumber, position.column);
