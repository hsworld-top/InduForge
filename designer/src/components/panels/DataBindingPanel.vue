    ...
    <div class="data-binding-panel p-3">
        ...
        <div v-else>
            <div class="mb-4">
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400">ID: {{ selectedComponent.id }}</div>
            </div>

            <!-- 绑定列表 -->
            <div class="space-y-3">
                <div v-for="(binding, path) in bindings" :key="path" class="p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
                    <div class="flex items-start justify-between mb-2">
                        ...
                        <el-button size="small" text type="danger" @click="removeBinding(path)">
                ...
                <div v-if="Object.keys(bindings).length === 0" class="text-center py-4 text-gray-500 dark:text-gray-400 text-sm">暂无数据绑定</div>
            </div>

            <!-- 添加绑定按钮 -->
            <el-button type="primary" size="small" class="w-full mt-3" @click="showAddDialog = true">
        ...
        <el-dialog v-model="showAddDialog" title="添加数据绑定" width="500px" :close-on-click-modal="false" :lock-scroll="false">
            <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="属性路径" prop="path">
                    <el-select v-model="form.path" placeholder="选择或输入属性路径" class="w-full" filterable allow-create>
                        <el-option-group label="属性 (props)">
                            <el-option v-for="prop in availableProps" :key="'props.' + prop" :label="'props.' + prop" :value="'props.' + prop" />
                        </el-option-group>
                        <el-option-group label="样式 (style)">
                            <el-option v-for="style in commonStyles" :key="'style.' + style" :label="'style.' + style" :value="'style.' + style" />
                        </el-option-group>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="快速选择">
                    <el-select v-model="quickSelect" placeholder="选择数据源" class="w-full" @change="onQuickSelect">
                        <el-option-group label="数据源">
                            <el-option v-for="ds in dataSources" :key="ds.id" :label="ds.id" :value="'data.' + ds.id" />
                        </el-option-group>
                        <el-option-group label="变量">
                            <el-option v-for="(value, key) in variables" :key="key" :label="key" :value="'vars.' + key" />
                        </el-option-group>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="预览">
                    <div class="p-2 bg-gray-50 dark:bg-gray-900 rounded text-sm">
                        <div v-if="previewError" class="text-red-500">错误: {{ previewError }}</div>
...
const rules = {
    path: [{ required: true, message: '请输入属性路径', trigger: 'blur' }],
...
const commonStyles = ['color', 'backgroundColor', 'opacity', 'width', 'height', 'left', 'top', 'transform', 'display', 'visibility'];
