        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1 min-w-0">
              ...
              <div v-if="ds.lastUpdate" class="text-xs text-gray-400 dark:text-gray-500 mt-1">
            ...
            <div class="flex items-center space-x-1 ml-2">
              <el-button
                size="small"
                text
                @click.stop="refreshDataSource(ds.id)"
                :loading="ds.status === 'loading'"
              >
          ...
          <div v-if="ds.error" class="mt-2 text-xs text-red-500 dark:text-red-400">
            错误: {{ ds.error }}
          </div>
        </div>
      </div>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <template v-if="form.type === 'dataCenter'">
          ...
          <template v-if="form.config.sourceType === 'tags'">
            <el-form-item label="连接" prop="config.connectionId">
              <el-select
                v-model="form.config.connectionId"
                placeholder="选择连接"
                class="w-full"
                filterable
        ...
        <template v-if="form.type === 'computed'">
          <el-form-item label="依赖" prop="config.dependencies">
            <el-select
              v-model="form.config.dependencies"
              placeholder="选择依赖的数据源"
              class="w-full"
              multiple
              filterable
            >
          ...
          <el-form-item label="计算函数" prop="config.compute">
            <el-input
              v-model="form.config.compute"
              type="textarea"
              :rows="6"
              placeholder="(sources) => ({ result: sources.ds1.value + sources.ds2.value })"
            />
          </el-form-item>
        </template>
