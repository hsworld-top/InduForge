...
export function resolveDownloadLinkConfig(
  ...
  const displayMode: DownloadLinkDisplayMode =
    rawDisplayMode === "link" ? "link" : "button";
  ...
  const triggerMode: DownloadLinkTriggerMode =
    rawTriggerMode === "single" ? "single" : "double";
...
export function buildDownloadLinkStyle(config: DownloadLinkConfig): Record<string, string> {
  ...
  return {
    ...
    textDecoration:
      config.displayMode === "link" && config.underline ? "underline" : "none",
