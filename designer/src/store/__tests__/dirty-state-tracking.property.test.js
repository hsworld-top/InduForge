 ...
 */
const pageSchemaWithComponentsArb = fc.array(componentSchemaArb, { minLength: 1, maxLength: 5 })
  .map(components => ({
    version: '2.0.0',
    meta: {
      id: 'test-page-id',
...
describe('Property 10: Dirty State Tracking', () => {
  ...
  it('should set isDirty to true when moving a component', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(componentSchemaArb, { minLength: 2, maxLength: 5 })
          .map(components => ({
            version: '2.0.0',
            meta: { id: 'test-page-id', name: 'Test Page', description: '' },
            config: {
              width: 1920, height: 1080, scaleMode: 'fit',
              backgroundColor: '#ffffff', gridSize: 10, snapToGrid: true, theme: 'light',
            },
            variables: {},
            dataSources: [],
            components,
  ...
  it('should keep isDirty true after multiple modifications', () => {
    fc.assert(
      fc.property(
        pageSchemaWithComponentsArb,
        fc.array(propUpdatesArb, { minLength: 1, maxLength: 5 }),
        (pageSchema, updatesList) => {
          const store = createStoreWithPage(pageSchema)
          const componentId = pageSchema.components[0].id
