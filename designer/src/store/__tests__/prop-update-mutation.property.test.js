 ...
 */
const componentSchemaArb = fc.tuple(componentIdArb, componentLabelArb, initialPropsArb, initialStyleArb)
  .map(([id, label, props, style]) => ({
    id,
    type: 'Button',
    label,
    locked: false,
  ...
  it('should correctly apply multiple prop updates in sequence', () => {
    fc.assert(
      fc.property(
        componentSchemaArb,
        fc.array(
          fc.tuple(propKeyArb, propValueArb),
