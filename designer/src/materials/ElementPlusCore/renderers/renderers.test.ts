...
const globalStubs = {
  ElSelect: {
    ...
    template: '<div class="el-select" @click="$emit(\'update:modelValue\', \'stopped\')"><slot /></div>',
  },
  ElOption: {
    props: ["label", "value", "disabled"],
    template: '<div class="el-option" :data-value="String(value)" :data-disabled="String(disabled)">{{ label }}</div>',
  },
  ElRadioGroup: {
    inheritAttrs: false,
    emits: ["update:modelValue"],
    template: '<div class="el-radio-group" @click="$emit(\'update:modelValue\', \'option2\')"><slot /></div>',
  },
  ElRadio: {
    props: ["label", "disabled"],
    template: '<label class="el-radio" :data-value="String(label)" :data-disabled="String(disabled)"><slot /></label>',
  },
  ElRadioButton: {
    props: ["label", "disabled"],
    template: '<label class="el-radio-button" :data-value="String(label)" :data-disabled="String(disabled)"><slot /></label>',
  },
  ElCheckboxGroup: {
    inheritAttrs: false,
    emits: ["update:modelValue"],
    template: '<div class="el-checkbox-group" @click="$emit(\'update:modelValue\', [\'b\'])"><slot /></div>',
  },
  ElCheckbox: {
    props: ["label", "disabled"],
    template: '<label class="el-checkbox" :data-value="String(label)" :data-disabled="String(disabled)"><slot /></label>',
  },
  ElCheckboxButton: {
    props: ["label", "disabled"],
    template: '<label class="el-checkbox-button" :data-value="String(label)" :data-disabled="String(disabled)"><slot /></label>',
  },
  ElInputNumber: {
    emits: ["update:modelValue"],
  ...
  ElTableColumn: {
    ...
    template: '<div class="el-table-column" :data-prop="prop" :data-width="String(width)" :data-align="align">{{ label }}</div>',
  },
};
