...
const pageRequiredFields = ['version', 'meta', 'config', 'variables', 'dataSources', 'components', 'permissions'];
const componentRequiredFields = ['id', 'type', 'label', 'locked', 'visible', 'style', 'props', 'bindings', 'events', 'animations', 'children'];
...
const validComponentSchemaArbitrary = fc.record({
  ...
  bindings: fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }), fc.string({ minLength: 1, maxLength: 50 })),
  events: fc.constant({}),
...
describe('Schema Validation Property Tests', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (schema, invalidScaleMode) => {
          ...
          const hasScaleModeError = result.errors.some(
            e => e.path === 'config.scaleMode'
          );
    ...
    fc.assert(
      fc.property(
        ...
        (schema, invalidTheme) => {
          ...
          const hasThemeError = result.errors.some(
            e => e.path === 'config.theme'
          );
    ...
    fc.assert(
      fc.property(
        ...
        (schema, invalidMode) => {
          ...
          const hasModeError = result.errors.some(
            e => e.path === 'mode'
          );
  ...
  test('validates invalid action type enum in action schema', () => {
    fc.assert(
      fc.property(
        fc.uuid(),
        invalidEnumArbitrary(validActionTypes),
        (id, invalidAction) => {
          ...
          const hasActionError = result.errors.some(
            e => e.path === 'action'
          );
