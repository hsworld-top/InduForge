...
const normalizeRuntimeRoleCode = (value) => {
  ...
  return normalizedCode.startsWith(DEFAULT_RUNTIME_ROLE_CODE_PREFIX)
    ? normalizedCode
    : `${DEFAULT_RUNTIME_ROLE_CODE_PREFIX}${normalizedCode}`
}
...
const extractActorId = (context = {}) =>
  normalizeText(
    context.creator?.id ||
      context.createdBy ||
      context.project?.createdBy ||
      context.project?.creator?.id,
  )
...
const findExistingBootstrapRuntimeUser = async ({ projectId, actorId, username, transaction }) => {
  if (actorId) {
...
const sanitizeRuntimeUser = (runtimeUser) => {
  ...
  const plainUser =
    typeof runtimeUser.toJSON === 'function' ? runtimeUser.toJSON() : { ...runtimeUser }
...
const normalizeRoleIds = (roleIds = []) => [
  ...new Set(
    (Array.isArray(roleIds) ? roleIds : []).map((item) => normalizeText(item)).filter(Boolean),
  ),
]
...
const buildRuntimeRoleSummary = (role) => {
  ...
  const bindingCount = new Set(bindings.map((binding) => binding.runtimeUserId).filter(Boolean))
    .size
...
const loadRuntimeUserSummary = async ({ projectId, runtimeUserId, transaction = null } = {}) => {
...
const loadRuntimeRoleSummary = async ({ projectId, roleId, transaction = null } = {}) => {
...
const assertAnotherActiveRuntimeAdminExists = async ({
  ...
  const hasRuntimeAdminRole = currentBindings.some(
    (binding) => binding.roleId === runtimeAdminRole.id,
  )
  ...
  const otherAdminIds = [
    ...new Set(
      adminBindings
        ...
        .filter(
          (bindingRuntimeUserId) => bindingRuntimeUserId && bindingRuntimeUserId !== runtimeUserId,
        ),
    ),
  ]
...
async function ensureRuntimeAdminBootstrap(input = {}, legacyOptions = {}) {
  const context =
    typeof input === 'string' ? { project: { id: input }, ...(legacyOptions || {}) } : input || {}
  const project = context.project || {}
  const creator = context.creator || project.creator || {}
  const transaction = context.transaction
  const resolvedProjectId = normalizeText(project.id || context.projectId || context.id)
  ...
  const roleDescription =
    normalizeText(context.roleDescription) || '工程运行态默认管理员角色，用于初始化首个可管理账号'
...
async function updateRuntimeUserStatus({ projectId, runtimeUserId, status, actorId = null } = {}) {
...
async function deleteRuntimeUser({ projectId, runtimeUserId } = {}) {
  if (!projectId || !runtimeUserId) {
    throw new AppError(ErrorCodes.VALIDATION_FAILED, 400, { message: '参数不完整' })
  }
...
async function createRuntimeRole({ projectId, actorId = null, code, name, description } = {}) {
  const normalizedCode = normalizeRuntimeRoleCode(code)
...
async function updateRuntimeRole({
  ...
  const nextCode = normalizeRuntimeRoleCode(code)
...
async function deleteRuntimeRole({ projectId, roleId, actorId = null } = {}) {
  const role = await ProjectRole.findOne({
    where: { projectId, id: roleId },
    include: buildRuntimeRoleInclude(),
  })
...
function buildEffectiveRoleGrantMap(roleGrants = []) {
  ...
  const finalizeGrantBucket = (localBucket, inheritedBucket = null) => {
    const effectiveAllowRoles = new Set(inheritedBucket ? inheritedBucket.allowRoles : [])
    const effectiveDenyRoles = new Set(inheritedBucket ? inheritedBucket.denyRoles : [])
      ...
      return [
        ...
        Object.fromEntries(
          [...resourceBuckets.entries()].map(([resourceId, actionBuckets]) => {
            ...
            const actionNames = new Set([...wildcardActionBuckets.keys(), ...actionBuckets.keys()])
            ...
            return [resourceId, Object.fromEntries(mergedActionBuckets.entries())]
          }),
        ),
      ]
    }),
...
module.exports = {
  DEFAULT_RUNTIME_ROLE_CODE_PREFIX,
