...
const SUPPORTED_DEPLOY_STATUS = new Set([
  'pending',
  'deploying',
  'running',
  'stopped',
...
const resolveDeployStatuses = (query = {}) => {
  return normalizeArrayQuery(query.deployStatus)
    .map((item) => item.toLowerCase())
    .filter((item) => SUPPORTED_DEPLOY_STATUS.has(item))
}

const resolveRuntimeModes = (query = {}) => {
  return normalizeArrayQuery(query.runtimeMode)
    .map((item) => item.toUpperCase())
    .filter((item) => SUPPORTED_RUNTIME_MODE.has(item))
}

const resolveNormalizedQuery = (query = {}) => {
  const page = resolvePaginationNumber(query.page, appConfig.pagination.defaultPage, 1)
...
const buildRuntimeSummary = (nodeDeployments = [], versionDeployments = []) => {
  ...
  normalized.forEach((deployment) => {
    ...
    const timeline = toSafeDateValue(
  ...
  const runtimeMode =
    pickLatestRuntimeMode(normalized) || pickLatestRuntimeMode(normalizedVersions) || 'DEV'
