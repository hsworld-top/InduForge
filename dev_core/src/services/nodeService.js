...
function normalizeRegisterError(error, nodeName) {
  if (error && error.name === 'SequelizeUniqueConstraintError') {
    ...
    const conflictField = Object.keys(error.fields || {})
      .join(',')
      .toLowerCase()
  ...
  async getPendingCommands(nodeId) {
    ...
    for (const pendingDeployment of pendingDeployments) {
      ...
      await NodeCommand.create({
        id: commandId,
        tenantId:
          pendingDeployment.tenantId ||
          (await Node.findByPk(nodeId, { attributes: ['tenantId'] })).tenantId,
        nodeId,
        deploymentId: pendingDeployment.id,
  ...
  async processCommandTimeouts() {
    ...
    for (const command of commands) {
      ...
      if (nextAttempts >= Number(command.maxAttempts || 3)) {
        ...
        if (nodeDeployment) {
          const tenantId = (
            await Node.findByPk(nodeDeployment.nodeId, { attributes: ['tenantId'] })
          )?.tenantId
          if (tenantId) {
