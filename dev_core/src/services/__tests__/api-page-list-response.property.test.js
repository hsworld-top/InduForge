 ...
 */
const pageNameArb = fc.string({ minLength: 1, maxLength: 200 }).filter((s) => s.trim().length > 0)
...
const pageArb = fc.record({
  ...
  type: pageTypeArb,
  parentId: fc.option(
    fc.constant(null).map(() => generateUUIDv4()),
    { nil: null },
  ),
...
function hasValidFieldTypes(page) {
  ...
  if (
    page.parentId !== null &&
    (typeof page.parentId !== 'string' || !UUID_V4_REGEX.test(page.parentId))
  ) {
    return false
  }
