...
describe('projectRuntimeAccessService', () => {
  ...
  test('binding 的 findOrCreate 遇到唯一冲突后会回查并复用已有记录', async () => {
    ...
    mockProjectUserRoleBinding.findOne
      ...
      .mockResolvedValueOnce(existingBinding)
  ...
  test('createRuntimeUser 会创建用户并绑定角色，且返回对象不含 passwordHash', async () => {
    ...
    mockProjectRuntimeUser.findOne.mockResolvedValueOnce(null).mockResolvedValueOnce({
      id: 'runtime-user-1',
      projectId: 'project-1',
      username: 'operator',
      displayName: '值班员',
  ...
  test('updateRuntimeUserStatus 只接受 active 或 disabled，并对外返回 active 或 disabled', async () => {
    ...
    mockProjectRuntimeUser.findOne.mockResolvedValueOnce(runtimeUserRecord).mockResolvedValueOnce({
      id: 'runtime-user-1',
      projectId: 'project-1',
      username: 'operator',
      displayName: '值班员',
  ...
  test('updateRuntimeUserStatus 在还有其他 active 管理员时允许禁用当前管理员', async () => {
    ...
    mockProjectRuntimeUser.findAll.mockResolvedValue([{ id: 'runtime-user-2', status: 'active' }])
  ...
  test('bindRuntimeUserRoles 不允许把工程里的最后一个管理员解绑干净', async () => {
    mockProjectRole.findAll.mockResolvedValue([{ id: 'role-viewer' }])
    mockProjectRole.findOne.mockResolvedValue({
      id: 'role-admin',
      code: DEFAULT_RUNTIME_ADMIN_ROLE_CODE,
      isSystem: true,
    ...
    mockProjectRole.findAll.mockResolvedValue([
      {
        ...
        name: '值班员',
        isSystem: false,
        userBindings: [{ runtimeUserId: 'runtime-user-1' }, { runtimeUserId: 'runtime-user-2' }],
