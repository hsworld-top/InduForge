...
class SocketService {
  ...
  setupEventHandlers() {
    this.io.on('connection', (socket) => {
      ...
      logger.info(`[SocketService] Client connected: ${socket.id}, project: ${projectId}`)

      // 鍔犲叆椤圭洰鎴块棿
      if (projectId) {
        socket.join(`project:${projectId}`)
        logger.info(`[SocketService] Client ${socket.id} joined room: project:${projectId}`)
      }
      ...
      socket.on('mqtt:subscribe', (data) => {
        ...
        if (subscriptionId) {
          ...
          console.log(`[SocketService] Client ${socket.id} joined room: ${roomName}`)
          logger.info(`[SocketService] Client ${socket.id} subscribed to: ${subscriptionId}`)
        }
      })
      ...
      socket.on('mqtt:unsubscribe', (data) => {
        ...
        if (subscriptionId) {
          ...
          logger.info(`[SocketService] Client ${socket.id} unsubscribed from: ${subscriptionId}`)
        }
      })
      ...
      socket.on('disconnect', (reason) => {
        logger.info(`[SocketService] Client disconnected: ${socket.id}, reason: ${reason}`)
      })
  ...
  broadcastConnectionStatus(projectId, connectionId, status) {
    ...
    logger.debug(`[SocketService] Broadcasted connection status to room: ${room}`)
  }
  ...
  broadcastSubscriptionStatus(projectId, subscriptionId, status) {
    ...
    logger.debug(`[SocketService] Broadcasted subscription status to room: ${room}`)
  }
  ...
  broadcastTagValueUpdate(tagId, valueData) {
    ...
    logger.debug(`[SocketService] Broadcasted tag value update to room: ${tagRoom}`)
  }
  ...
  setupTagSubscription(socket) {
    ...
    socket.on('mqtt:tag:subscribe', (data) => {
      ...
      if (tagId) {
        ...
        logger.info(`[SocketService] Client ${socket.id} subscribed to tag: ${tagId}`)
      }
    })
    ...
    socket.on('mqtt:tag:unsubscribe', (data) => {
      ...
      if (tagId) {
        ...
        logger.info(`[SocketService] Client ${socket.id} unsubscribed from tag: ${tagId}`)
      }
    })
  }
    ...
    socket.on('datapoint:subscribe', (data) => {
      ...
      paths
        ...
        .forEach((path) => {
          ...
          socket.join(room)
          logger.info(`[SocketService] Client ${socket.id} subscribed to datapoint: ${path}`)
        })
    })
    ...
    socket.on('datapoint:unsubscribe', (data) => {
      ...
      paths
        ...
        .forEach((path) => {
          ...
          logger.info(`[SocketService] Client ${socket.id} unsubscribed from datapoint: ${path}`)
        })
    })
  }
  ...
  setupOpsSubscription(socket) {
    socket.on('ops:subscribe', (data) => {
      ...
      if (tenantId) {
        ...
        socket.join(room)
        logger.info(`[SocketService] Client ${socket.id} subscribed to ops: ${tenantId}`)
      }
    })
    ...
    socket.on('ops:unsubscribe', (data) => {
      ...
      if (tenantId) {
        ...
        logger.info(`[SocketService] Client ${socket.id} unsubscribed from ops: ${tenantId}`)
      }
    })
  }
  ...
  broadcastNodeMetrics(tenantId, nodeId, metrics) {
    if (!this.io) return
    const room = `ops:tenant:${tenantId}`
    this.io.to(room).emit('ops:node:metrics', { nodeId, metrics, timestamp: Date.now() })
  }
  ...
  broadcastNodeStatus(tenantId, nodeId, status) {
    if (!this.io) return
    const room = `ops:tenant:${tenantId}`
    this.io.to(room).emit('ops:node:status', { nodeId, status, timestamp: Date.now() })
  }
    ...
    socket.on('datapoint:subscribe', (data) => {
      ...
      targetPaths
        ...
        .forEach((normalized) => {
          ...
          socket.join(room)
          logger.info(`[SocketService] Client ${socket.id} subscribed to datapoint: ${normalized}`)
        })
    })
