  ...
  try {
    ...
    if (!deploymentId || !status) {
      return ApiResponse.error(
        ...
        { message: 'deploymentId 和 status 不能为空' },
        200,
      )
    }

    const result = await nodeService.updateDeploymentStatus(
      nodeId,
      deploymentId,
      {
        status,
...
router.post('/:nodeId/offline', async (req, res) => {
  try {
    ...
    const result = await nodeService.offline(
      nodeId,
      { reason: reason || 'agent_shutdown' },
      registrationToken,
    )
    return ApiResponse.success(res, result)
  } catch (error) {
