 ...
 */
router.post(
  '/project/:projectId/deploy-dev',
  authenticate,
  requireCapability('deploy:execute'),
  async (req, res) => {
    try {
      const { projectId } = req.params
      ...
      return ApiResponse.success(
        ...
        'operation_success',
        { message: `DEV部署任务已下发：${successCount} 个成功，${failCount} 个失败` },
      )
    } catch (error) {
  ...
  try {
    ...
    if (mode === 'DEV') {
      return ApiResponse.error(
        ...
        ErrorCodes.VALIDATION_FAILED,
        {
          message: 'DEV模式无需版本，请调用 /api/v1/deployments/project/:projectId/deploy-dev',
        },
        200,
      )
    }
 ...
 */
router.post(
  '/node-deployment/:id/start',
  authenticate,
  requireCapability('runtime:operate'),
  async (req, res) => {
    try {
      const { id } = req.params
 ...
 * @access 工程成员
 */
router.get('/project/:projectId/node/:nodeId/mode', authenticate, async (req, res) => {
  try {
    const { projectId, nodeId } = req.params
    await checkProjectAccess(req, projectId)
    const mode = await deploymentService.getNodeProjectMode(projectId, nodeId)
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await deploymentService.rollback(nodeId, deploymentId, deployedBy)
