...
const buildProjectSnapshot = (datacenter = {}) => {
  ...
  const relationalConfigMap = new Map(
    relationalConfigs.filter((item) => item?.connectionId).map((item) => [item.connectionId, item]),
  )
  const mqttConfigMap = new Map(
    mqttConfigs.filter((item) => item?.connectionId).map((item) => [item.connectionId, item]),
  )
  ...
  validate(
    Joi.object({
      query: Joi.object({
        ...
        runtimeMode: Joi.alternatives()
          .try(
            Joi.string().valid('DEV', 'RELEASE'),
            Joi.array().items(Joi.string().valid('DEV', 'RELEASE')),
          )
          .optional(),
        deployStatus: Joi.alternatives()
          .try(
        ...
        sortBy: Joi.string()
          .valid(...PROJECT_OVERVIEW_SORT_FIELDS)
          .optional(),
        sortField: Joi.string()
          .valid(...PROJECT_OVERVIEW_SORT_FIELDS)
          .optional(),
        sortOrder: Joi.string()
          .valid(...PROJECT_OVERVIEW_SORT_ORDERS, 'asc', 'desc')
          .optional(),
        order: Joi.string()
          .valid(...PROJECT_OVERVIEW_SORT_ORDERS, 'asc', 'desc')
          .optional(),
...
router.put(
  '/:id/tags',
  authenticateToken,
  requireResourceOwnership('project'),
  validate(
    Joi.object({
      params: Joi.object({
  ...
  async (req, res) => {
    try {
      ...
      const tagIds = [
        ...new Set(
          (Array.isArray(req.body.tagIds) ? req.body.tagIds : []).map((item) => String(item)),
        ),
      ]
  ...
  async (req, res) => {
    try {
      ...
      const pageIndex = pages.map((page) => {
        ...
        const file =
          page.type === 'page' || page.type === 'dialog' ? `${safeName}-${page.id}.json` : null
        return {
          id: page.id,
          name: page.name,
          type: page.type,
      ...
      archive.append(JSON.stringify(snapshot.connections, null, 2), {
        name: 'datacenter/connections.json',
      })
      archive.append(JSON.stringify(snapshot.relationalConfigs, null, 2), {
        name: 'datacenter/relational-configs.json',
      })
      archive.append(JSON.stringify(snapshot.queries, null, 2), { name: 'datacenter/queries.json' })
      archive.append(JSON.stringify(snapshot.mqttConfigs, null, 2), {
        name: 'datacenter/mqtt-configs.json',
      })
      archive.append(JSON.stringify(snapshot.mqttSubscriptions, null, 2), {
        name: 'datacenter/mqtt-subscriptions.json',
      })
      archive.append(JSON.stringify(snapshot.mqttTagGroups, null, 2), {
        name: 'datacenter/mqtt-tag-groups.json',
      })
      archive.append(JSON.stringify(snapshot.mqttTags, null, 2), {
        name: 'datacenter/mqtt-tags.json',
      })
      archive.append(JSON.stringify(snapshot.datapoints, null, 2), {
        name: 'datacenter/datapoints.json',
      })
      archive.append(JSON.stringify(artifact || {}, null, 2), { name: 'datacenter/artifact.json' })
  ...
  validate(
    Joi.object({
      body: Joi.object({
        ...
        description: Joi.string().allow('').optional(),
        colorTag: Joi.string()
          .valid('#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#6b7280')
          .optional(),
      }).required(),
    }),
  ),
...
router.get(
  '/:id/runtime-users',
  authenticateToken,
  requireRuntimeProjectManagement,
  validate(
    Joi.object({
      params: Joi.object({ id: Joi.string().uuid().required() }),
    }),
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      logger.error('Update runtime user status error', {
        error: error.message,
        requestId: req.requestId,
      })
      return respondRouteError(res, error, ErrorCodes.INTERNAL_SERVER_ERROR, 500)
...
router.delete(
  '/:id/runtime-users/:runtimeUserId',
  authenticateToken,
  requireRuntimeProjectManagement,
  validate(
    Joi.object({
...
router.post(
  '/:id/runtime-users/:runtimeUserId/reset-password',
  authenticateToken,
  requireRuntimeProjectManagement,
  validate(
    Joi.object({
  ...
  async (req, res) => {
    try {
      ...
      logger.error('Reset runtime user password error', {
        error: error.message,
        requestId: req.requestId,
      })
      return respondRouteError(res, error, ErrorCodes.INTERNAL_SERVER_ERROR, 500)
...
router.put(
  '/:id/runtime-users/:runtimeUserId/roles',
  authenticateToken,
  requireRuntimeProjectManagement,
  validate(
    Joi.object({
  ...
  async (req, res) => {
    try {
      ...
      logger.error('Bind runtime user roles error', {
        error: error.message,
        requestId: req.requestId,
      })
      return respondRouteError(res, error, ErrorCodes.INTERNAL_SERVER_ERROR, 500)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.error('Get project delete impact error', {
        error: error.message,
        requestId: req.requestId,
      })
      return ApiResponse.error(res, ErrorCodes.INTERNAL_SERVER_ERROR, {}, 500)
