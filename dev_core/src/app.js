...
function buildApp() {
    ...
    app.use(
      ...
      (req, res, next) => {
        ...
        res.setHeader(
          ...
          "frame-ancestors *; script-src 'self' 'unsafe-eval' 'unsafe-inline'; connect-src *",
        )
  ...
  if (fs.existsSync(viewsPath) && fs.existsSync(viewsIndexPath)) {
    ...
    app.use((req, res, next) => {
      ...
      res.setHeader(
        'Content-Security-Policy',
        `default-src 'self'; frame-src 'self' ${localNodeAgentFrontOrigins.join(' ')}; script-src 'self' 'unsafe-eval' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src 'self'`,
      )
      // 返回前端 index.html
