...
func TestFoundationResourceRefsAreInternalAndDeterministic(t *testing.T) {
	...
	if err != nil || nats["service"] != "nats://nats:4222" || nats["secretKey"] != "nats-token" || nats["authMode"] != "environment-token+project-subject-isolation" {
		t.Fatalf("nats refs invalid: %v %#v", err, nats)
	}
	postgres, err := foundationResourceRefs(environmentID, "if_history")
	if err != nil || postgres["service"] != "postgres:5432" || postgres["database"] != "induforge_runtime" || postgres["adminUser"] != "postgres" || postgres["secretKey"] != "postgres-password" || !strings.HasPrefix(postgres["schemaPrefix"], "runtime_") {
