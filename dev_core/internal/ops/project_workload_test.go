...
func TestRenderProjectWorkloadManifestSeparatesRolesAndHostPort(t *testing.T) {
	...
	for _, expected := range []string{"image: induforge/collector-engine:1.0.0", `command: ["collector_artifact_unpack"]`, `command: ["industrial_collector"]`, `"--artifact"`, `"--binding"`, `"--index"`, `"--listen"`, "mountPath: /etc/induforge/collector/secrets", `emptyDir: {sizeLimit: "1Gi"}`, "maxUnavailable: 0"} {
		if err != nil || !strings.Contains(collector, expected) {
			t.Fatalf("collector workload missing %q: %v\n%s", expected, err, collector)
		}
	}
	if strings.Contains(collector, "runtime-binding-prepare") || strings.Contains(collector, `command: ["runtime-engine"]`) || strings.Contains(collector, "if-runtime-provisioner") || strings.Contains(collector, "--secrets-dir") || strings.Contains(collector, "--wal-dir") {
