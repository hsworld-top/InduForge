...
func bootstrapCommand(templateID string) string {
	...
	agents := "# 开发工作区协作规则\n\n这是工程内置的可编辑规则文件。开始工作前请先阅读本文件、`.workspace/context/current/README.md`（若存在）和实际工程的 `package.json`。\n\n## 开发顺序\n\n1. 先确认当前工程技术栈、启动命令和已有目录结构，再修改源码。\n2. 优先复用工程已有组件、Runtime SDK 和数据中心接口，不要猜测接口或对象库 URL。\n3. 涉及数据点时先阅读 `.workspace/context/current/points/manifest.json` 及按需的 JSON/Markdown 分片；运行时仍以服务端权限和契约为准。\n4. 视频、大文件、多人共享或需要后端处理的静态文件使用工程对象库；小型页面私有图片和图标可随源码发布。对象库操作使用项目文件 API 和 assetId，不要把对象 key 写入源码。\n5. 修改后运行与改动相关的测试、类型检查和构建，并说明未验证的浏览器流程。\n\n用户可以继续修改本文件以适配自己的团队规范。\n"
	agentsShell := strings.ReplaceAll(strings.ReplaceAll(agents, "\\", "\\\\"), "'", "'\\\"'\\\"'")
	return "test -z \"$(find /source -mindepth 1 -maxdepth 1 ! -name .workspace -print -quit)\"; if test -e /source/.workspace; then test -d /source/.workspace/context && test -z \"$(find /source/.workspace -mindepth 1 -maxdepth 1 ! -name context -print -quit)\"; fi; staging=/source/.workspace-initialize; mkdir \"$staging\"; cp -a /opt/induforge/templates/" + templateID + "/. \"$staging\"/; rm -rf \"$staging/node_modules\" \"$staging/dist\" \"$staging/.pnpm-store\"; mkdir -p \"$staging/.workspace\" /source/.workspace; chmod u+rwx \"$staging/.workspace\"; printf '%s\\n' '{\"version\":1,\"templateId\":\"" + templateID + "\"}' > \"$staging/.workspace/project.json\"; printf '%s' '" + agentsShell + "' > \"$staging/AGENTS.md\"; find \"$staging\" -mindepth 1 -maxdepth 1 ! -name .workspace -exec mv {} /source/ \\;; find \"$staging/.workspace\" -mindepth 1 -maxdepth 1 -exec mv {} /source/.workspace/ \\;; rmdir \"$staging/.workspace\" \"$staging\""
}
...
func (c *dockerFrontendClient) Run(ctx context.Context, s dockerFrontendSpec) error {
	...
	}{Image: s.Image, Entrypoint: []string{"/bin/sh", "-ec"}, Cmd: s.Command, Env: []string{"CI=true", "PNPM_CONFIG_STORE_DIR=/tmp/pnpm-store", "XDG_CACHE_HOME=/tmp/pnpm-cache", "HOME=/tmp/home", "COREPACK_HOME=/tmp/corepack"}, User: "1000:1000", WorkingDir: "/tmp", Labels: map[string]string{"com.induforge.managed": "true", "com.induforge.role": "release-frontend-build"}, HostConfig: host{NetworkMode: "none", ReadonlyRootfs: true, CapDrop: []string{"ALL"}, SecurityOpt: []string{"no-new-privileges:true"}, PidsLimit: 256, Memory: s.MemoryBytes, NanoCPUs: s.NanoCPUs, Tmpfs: map[string]string{"/tmp": "rw,noexec,nosuid,size=128m"}}}
