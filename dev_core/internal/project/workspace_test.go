...
func TestEmbeddedRuntimeSDKMatchesCanonicalSource(t *testing.T) {
	for _, name := range []string{"package.json", "index.js", "index.d.ts"} {
		embedded, err := workspaceTemplate.ReadFile(filepath.ToSlash(filepath.Join("template", ".induforge", "runtime-sdk", name)))
		if err != nil {
