...
JOIN projects p ON p.id = r.project_id
LEFT JOIN project_role_grants g ON g.role_id = r.id
WHERE r.project_id = sqlc.arg(project_id) AND p.tenant_id = sqlc.arg(tenant_id) AND p.status <> 'deleted' AND (sqlc.arg(keyword)::text = '' OR r.name ILIKE '%' || sqlc.arg(keyword) || '%' OR r.code ILIKE '%' || sqlc.arg(keyword) || '%')
GROUP BY r.id
ORDER BY r.is_builtin DESC, r.created_at, r.id LIMIT sqlc.arg(page_limit) OFFSET sqlc.arg(page_offset);

-- name: CountRuntimeRoles :one
SELECT count(*) FROM project_roles r JOIN projects p ON p.id = r.project_id WHERE r.project_id = sqlc.arg(project_id) AND p.tenant_id = sqlc.arg(tenant_id) AND p.status <> 'deleted' AND (sqlc.arg(keyword)::text = '' OR r.name ILIKE '%' || sqlc.arg(keyword) || '%' OR r.code ILIKE '%' || sqlc.arg(keyword) || '%');


-- name: PageRuntimeUsers :many
SELECT u.id, u.project_id, u.username, u.display_name, u.email, u.status, u.is_builtin_admin,
       u.last_login_at, u.password_changed_at, u.created_at, u.updated_at,
       COALESCE(jsonb_agg(DISTINCT jsonb_build_object('id', r.id, 'code', r.code, 'name', r.name, 'status', r.status, 'isBuiltin', r.is_builtin)) FILTER (WHERE r.id IS NOT NULL), '[]'::jsonb) AS roles
FROM project_runtime_users u
...
LEFT JOIN project_user_role_bindings b ON b.runtime_user_id = u.id
LEFT JOIN project_roles r ON r.id = b.role_id
WHERE u.project_id = sqlc.arg(project_id) AND p.tenant_id = sqlc.arg(tenant_id) AND p.status <> 'deleted' AND (sqlc.arg(keyword)::text = '' OR u.username ILIKE '%' || sqlc.arg(keyword) || '%' OR u.display_name ILIKE '%' || sqlc.arg(keyword) || '%')
GROUP BY u.id
ORDER BY u.is_builtin_admin DESC, u.created_at, u.id LIMIT sqlc.arg(page_limit) OFFSET sqlc.arg(page_offset);

-- name: CountRuntimeUsers :one
SELECT count(*) FROM project_runtime_users u JOIN projects p ON p.id = u.project_id WHERE u.project_id = sqlc.arg(project_id) AND p.tenant_id = sqlc.arg(tenant_id) AND p.status <> 'deleted' AND (sqlc.arg(keyword)::text = '' OR u.username ILIKE '%' || sqlc.arg(keyword) || '%' OR u.display_name ILIKE '%' || sqlc.arg(keyword) || '%');
