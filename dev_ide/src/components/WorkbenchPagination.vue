<template>
  <div class="workbench-pagination">
    <p class="pagination-summary">{{ resolvedSummary }}</p>

    <div class="pagination-controls">
      <span class="pagination-label">{{ pageSizeLabel }}</span>
      <el-popover placement="top" :width="100" trigger="click">
        ...
        <div class="pagination-size-menu">
          <button
            ...
            :class="{ 'is-active': safeLimit === sizeOption }"
      ...
      <div class="pagination-nav-group">
        <button type="button" class="pagination-nav-btn" :disabled="!canPrev" @click="handlePrev">
          <el-icon><ArrowLeft /></el-icon>
        </button>
        <button type="button" class="pagination-nav-btn" :disabled="!canNext" @click="handleNext">
          <el-icon><ArrowRight /></el-icon>
        </button>
      </div>

      <span class="pagination-page-info">
        {{ resolvedPageIndicator }}
      </span>
    </div>
  </div>
</template>
...
const resolvedSummary = computed(() => {
  ...
  const start = (safePage.value - 1) * safeLimit.value + 1
  const end = Math.min(safePage.value * safeLimit.value, safeTotal.value)
  return `${start}-${end} / 共 ${safeTotal.value} 条`
})

const resolvedPageIndicator = computed(() => {
  if (props.pageIndicator) {
    return props.pageIndicator
  }
  return `${safeTotalPages.value <= 0 ? 0 : safePage.value} / ${safeTotalPages.value}`
})
