  ...
  <div>
    ...
    <transition name="slide">
      <aside v-if="visible" class="tm-drawer bg-white dark:bg-slate-900 shadow-2xl">
        <header class="tm-drawer-head px-8 py-6 border-b border-slate-100 dark:border-slate-800/60 flex justify-between items-start bg-slate-50/50 dark:bg-slate-900/50 backdrop-blur-sm sticky top-0 z-20">
          <div>
          ...
          <button class="text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-slate-300 transition-all rounded-xl p-2.5 -mr-2" type="button" @click="closeDrawer">
        ...
        <nav class="flex border-b border-slate-100 dark:border-slate-800/60 px-8 pt-4 gap-8 bg-white dark:bg-slate-900 sticky top-[93px] z-10">
          <button
            type="button"
            v-for="tab in tabs"
            :key="tab.k"
          ...
          >
            {{ tab.l }}
            <span v-if="curTab === tab.k" class="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 dark:bg-blue-500 rounded-t-full shadow-[0_-2px_8px_rgba(37,99,235,0.4)]"></span>
          </button>
        </nav>
        ...
        <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
          ...
          <section v-show="curTab === 'basic'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                  {{ t("tenantManagement.tenantName") }} <span class="text-rose-500 ml-1">*</span>
                </label>
                <input v-model="form.name" required class="fm-input" :placeholder="t('tenantManagement.tenantName')" />
              </div>
              <div>
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                  {{ t("tenantManagement.tenantCode") }} <span class="text-rose-500 ml-1">*</span>
                </label>
                <input v-model="form.code" required :disabled="!isAdd" class="fm-input disabled:opacity-50 disabled:bg-slate-50 dark:disabled:bg-slate-800/50" placeholder="e.g. my-tenant-01" />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea v-model="form.description" rows="3" class="fm-input fm-ta" placeholder="..."></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input v-model="form.contactEmail" type="email" class="fm-input" placeholder="name@example.com" />
              </div>
              <div>
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
            ...
            <div v-if="!isAdd" class="p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
              <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                {{ t("tenantManagement.status") }}
              </label>
              <select v-model="form.status" class="fm-input cursor-pointer bg-white dark:bg-slate-900">
          ...
          <section v-show="curTab === 'company'" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="md:col-span-2">
                ...
                <input v-model="form.companyWebsite" type="url" class="fm-input" placeholder="https://" />
              </div>
              <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                  {{ t("tenantManagement.icpNumber") }}
                </label>
                <input v-model="form.icpNumber" class="fm-input" :placeholder="t('tenantManagement.icpPlaceholder')" />
              </div>
            </div>

            <div class="h-px bg-slate-200/60 dark:bg-slate-800 my-8"></div>
            
            <div class="bg-slate-50/50 dark:bg-slate-900/30 p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6">
                <label class="flex items-center gap-3 text-sm font-medium text-slate-600 dark:text-slate-400 cursor-pointer hover:bg-white dark:hover:bg-slate-800/50 p-2.5 rounded-xl transition-all shadow-sm border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                  <input v-model="form.showCompanyName" type="checkbox" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" />
                  <span>{{ t("tenantManagement.showCompanyName") }}</span>
                </label>
                <label class="flex items-center gap-3 text-sm font-medium text-slate-600 dark:text-slate-400 cursor-pointer hover:bg-white dark:hover:bg-slate-800/50 p-2.5 rounded-xl transition-all shadow-sm border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                  <input v-model="form.showCompanyPhone" type="checkbox" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" />
                  <span>{{ t("tenantManagement.showCompanyPhone") }}</span>
                </label>
                <label class="flex items-center gap-3 text-sm font-medium text-slate-600 dark:text-slate-400 cursor-pointer hover:bg-white dark:hover:bg-slate-800/50 p-2.5 rounded-xl transition-all shadow-sm border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                  <input v-model="form.showCompanyAddress" type="checkbox" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" />
                  <span>{{ t("tenantManagement.showCompanyAddress") }}</span>
                </label>
                <label class="flex items-center gap-3 text-sm font-medium text-slate-600 dark:text-slate-400 cursor-pointer hover:bg-white dark:hover:bg-slate-800/50 p-2.5 rounded-xl transition-all shadow-sm border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                  <input v-model="form.showCompanyWebsite" type="checkbox" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" />
                  <span>{{ t("tenantManagement.showCompanyWebsite") }}</span>
                </label>
                <label class="flex items-center gap-3 text-sm font-medium text-slate-600 dark:text-slate-400 cursor-pointer hover:bg-white dark:hover:bg-slate-800/50 p-2.5 rounded-xl transition-all shadow-sm border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                  <input v-model="form.showIcp" type="checkbox" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" />
          ...
          <section v-show="curTab === 'brand'" class="space-y-8">
            <div class="flex flex-col gap-3">
              <div>
                <p class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ t("tenantManagement.logo") }}</p>
                <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">推荐 200×200，PNG / JPG / SVG</p>
              </div>
              <el-upload
              ...
              >
                <div class="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 hover:text-blue-500 transition-colors">
            ...
            <div class="flex flex-col gap-3">
              <div>
                <p class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ t("tenantManagement.loginBackground") }}</p>
                <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">推荐 1920×1080，JPG 或 PNG</p>
              </div>
              <el-upload
            ...
            <div class="flex items-center gap-5 mt-8 p-5 border border-dashed border-slate-300 dark:border-slate-700 rounded-2xl bg-slate-50 dark:bg-slate-800/30">
              <div class="relative">
                <img :src="logoPreviewUrl" alt="logo" class="w-14 h-14 rounded-xl object-cover border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm" />
        ...
        <footer class="flex items-center justify-end gap-3 px-8 py-5 border-t border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/50 backdrop-blur-sm z-20">
          <button type="button" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-slate-100 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700 dark:focus:ring-slate-800 transition-all shadow-sm active:scale-[0.98]" @click="closeDrawer">
    ...
    >
      <div class="flex justify-center">
        <img :src="previewImageUrl" alt="preview" class="max-w-full max-h-[70vh] rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700" />
      </div>
    </el-dialog>
  </div>
</template>
...
const uploadBackgroundRequest = async (options: any) => {
  ...
  try {
    ...
    emit('updateListBrand', { code: form.code, loginBackgroundUrl: res.data.fileUrl })
