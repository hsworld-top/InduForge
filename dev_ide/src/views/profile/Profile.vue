<template>
  <div class="p-6 max-w-[1200px] mx-auto w-full font-sans antialiased" :class="{ 'px-2 py-2 max-h-[70vh] overflow-y-auto custom-scrollbar': embedded }">
    <!-- 头部资料卡片 -->
    <div class="relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm mb-6 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all">
      <!-- 装饰性背景光晕 -->
      ...
      <div class="flex items-center gap-6 relative z-10">
        <el-avatar :size="80" :src="profile.avatarUrl" class="border-4 border-white dark:border-slate-800 shadow-md bg-gradient-to-br from-blue-100 to-indigo-50 dark:from-blue-900/40 dark:to-indigo-900/40 text-blue-600 dark:text-blue-400 font-extrabold text-2xl">
          {{ userInitials }}
        </el-avatar>
        <div>
          <h2 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ profile.username || '-' }}</h2>
          <div class="flex flex-wrap items-center gap-3 mt-2.5">
            <span class="px-2.5 py-1 text-xs font-bold text-blue-700 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 rounded-lg">{{ getRoleLabel(profile.role) }}</span>
      ...
      <div class="flex flex-wrap items-center gap-3 relative z-10">
        ...
        >
          <button type="button" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 focus:ring-4 focus:ring-slate-100 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700 transition-all shadow-sm active:scale-[0.98]">
            {{ t('profile.uploadAvatar') }}
          </button>
        </el-upload>
        <button type="button" class="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 focus:ring-4 focus:ring-slate-100 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700 transition-all shadow-sm active:scale-[0.98]" @click="loadProfile" :disabled="loading">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
      ...
      <!-- 账号信息 -->
      <div class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm">
        <h3 class="text-lg font-extrabold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
          <div class="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg">
            <svg class="w-5 h-5 text-slate-500 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
          </div>
          {{ t('profile.accountInfo') }}
        </h3>
        
        <div class="space-y-4">
          <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/60">
            <span class="text-sm font-semibold text-slate-500 dark:text-slate-400">{{ t('profile.username') }}</span>
            <span class="text-sm font-bold text-slate-900 dark:text-white">{{ profile.username || '-' }}</span>
          </div>
          <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/60">
            <span class="text-sm font-semibold text-slate-500 dark:text-slate-400">{{ t('profile.role') }}</span>
            <span class="text-sm font-bold text-slate-900 dark:text-white">{{ getRoleLabel(profile.role) }}</span>
          </div>
          <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/60 cursor-pointer hover:border-blue-300 dark:hover:border-blue-700 transition-colors group" @click="startEmailEdit">
            <span class="text-sm font-semibold text-slate-500 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ t('profile.email') }}</span>
            <div class="flex items-center gap-2">
              ...
              <div v-else class="flex items-center gap-2">
                <span class="text-sm font-bold text-slate-900 dark:text-white">{{ profile.email || '-' }}</span>
                <svg class="w-4 h-4 text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm">
        ...
        <el-form ref="preferencesFormRef" :model="preferencesForm" :rules="preferencesRules" label-position="top">
          <el-form-item :label="t('profile.language')" prop="language">
            <el-select v-model="preferencesForm.language" style="width: 100%" size="large" class="modern-select">
              <el-option
          ...
          <el-form-item :label="t('profile.theme')" prop="theme">
            <el-select v-model="preferencesForm.theme" style="width: 100%" size="large" class="modern-select">
              <el-option
          ...
          <div class="pt-4">
            <button type="button" class="w-full px-5 py-3 text-sm font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-900/40 transition-all shadow-md active:scale-[0.98] disabled:opacity-70 flex justify-center items-center gap-2" :disabled="savingPreferences" @click="handleSavePreferences">
              <svg v-if="savingPreferences" class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
      ...
      <!-- 修改密码 -->
      <div class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm xl:col-span-2">
        <h3 class="text-lg font-extrabold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
          <div class="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg">
            <svg class="w-5 h-5 text-slate-500 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
          </div>
        ...
        <el-form ref="passwordFormRef" :model="passwordForm" :rules="passwordRules" label-position="top">
          ...
          <div class="pt-4 flex justify-end">
            <button type="button" class="px-6 py-3 text-sm font-semibold text-white bg-slate-800 dark:bg-slate-700 rounded-xl hover:bg-slate-900 dark:hover:bg-slate-600 focus:ring-4 focus:ring-slate-500/20 transition-all shadow-md active:scale-[0.98] disabled:opacity-70 flex justify-center items-center gap-2" :disabled="savingPassword" @click="handleChangePassword">
...
:deep(.modern-select .el-input__wrapper.is-focus) {
  box-shadow: 0 0 0 1px #3b82f6 inset, 0 0 0 4px rgba(59, 130, 246, 0.1);
...
html.dark :deep(.modern-select .el-input__wrapper.is-focus) {
  box-shadow: 0 0 0 1px #3b82f6 inset, 0 0 0 4px rgba(59, 130, 246, 0.2);
}
