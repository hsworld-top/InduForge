<template>
  ...
  >
    <!-- 顶部：Logo 与 收起按钮 -->
    <div
      class="h-14 px-4 flex items-center justify-between border-b"
    ...
    <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
      <nav>
        <button
          v-if="hasTabPermission('dashboard')"
          @click="handleOpenTab('dashboard')"
          :class="[
        ...
        <button
          v-if="hasTabPermission('tenant-management')"
          @click="handleOpenTab('tenant-management')"
          :class="[
            'w-full h-10 flex items-center px-3 gap-3 rounded-lg transition-colors mb-1',
        ...
        <button
          v-if="hasTabPermission('user-management')"
          @click="handleOpenTab('user-management')"
          :class="[
            'w-full h-10 flex items-center px-3 gap-3 rounded-lg transition-colors mb-1',
        ...
        <button
          v-if="hasTabPermission('project-management')"
          @click="handleOpenTab('project-management')"
          :class="[
            'w-full h-10 flex items-center px-3 gap-3 rounded-lg transition-colors mb-1',
        ...
        <button
          v-if="hasTabPermission('ops-management')"
          @click="handleOpenTab('ops-management')"
          :class="[
            'w-full h-10 flex items-center px-3 gap-3 rounded-lg transition-colors mb-1',
        ...
        <button
          v-if="hasTabPermission('system-logs')"
          @click="handleOpenTab('system-logs')"
          :class="[
            'w-full h-10 flex items-center px-3 gap-3 rounded-lg transition-colors mb-1',
    ...
    <div class="p-3" :class="isDark ? 'border-gray-800' : 'border-gray-100'">
      ...
      >
        <div
          class="flex items-center gap-2.5 h-[44px] px-2 w-full rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-700"
        >
          <img
            v-if="userAvatarUrl"
            :src="userAvatarUrl"
          ...
          <div
            v-else
            class="w-7 h-7 bg-blue-600 rounded-full flex items-center justify-center text-white text-xs font-medium shadow-sm shrink-0"
          >
            {{ userInitials }}
          </div>
          <div class="flex-1 min-w-0">
            <p
              class="text-[13px] leading-tight font-medium text-gray-900 dark:text-gray-100 truncate"
            >
              {{ username }}
            </p>
            <p class="text-[11px] leading-tight mt-0.5 text-gray-500 dark:text-gray-400 truncate">
        ...
        <template #dropdown>
          <el-dropdown-menu
            class="w-[220px] shadow-xl rounded-xl border border-gray-100 dark:border-gray-700"
          >
            ...
            <el-dropdown-item divided command="toggleTheme">
              <span
                class="flex items-center justify-between w-full text-gray-600 dark:text-gray-300 text-[13px]"
              >
            ...
            <el-dropdown-item class="!p-0" divided @click.prevent.stop>
              <el-popover
                ...
                trigger="hover"
                width="140"
                popper-class="!p-1.5 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700"
                :show-arrow="false"
                :offset="12"
              >
                <template #reference>
                  <div
                    class="flex items-center justify-between w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                  >
                    <span
                      class="flex items-center gap-2 text-gray-600 dark:text-gray-300 text-[13px]"
                    >
                      <svg
                        class="w-3.5 h-3.5"
                        fill="none"
                ...
                <div class="flex flex-col gap-0.5">
                  <button
                    @click="handleUserMenuCommand('lang-zh')"
                    :class="[
                      'w-full text-left px-3 py-2 rounded-lg text-[13px] hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors',
                      currentLanguage === 'zh'
                  ...
                  <button
                    @click="handleUserMenuCommand('lang-en')"
                    :class="[
                      'w-full text-left px-3 py-2 rounded-lg text-[13px] hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors',
                      currentLanguage === 'en'
                        ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/20 font-medium'
