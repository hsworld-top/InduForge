...
> 当前进度：M1（编辑器内核）已完成；仅支持 API 模式，不规划 Bridge。
...
- **布局组件**: 响应式布局、分栏*1、分栏*2、分栏*4
- **UI 组件**: PC 端组件集合（按钮、文本、图片、输入框、选择器、开关、表格、树、下拉菜单、导航菜单、单选框、多选框、级联选择器、标签页等）
- **自定义组件**: 支持组件封装和复用
...
- 事件/绑定融合：组件选中时展示组件事件，点击画布空白或网格时展示页面绑定内容
- 页面绑定完善：支持创建时/存在时/关闭时脚本、定时器、变量改变配置，并沿用统一脚本编辑器规则
- 变量页签：新增页面变量管理、编辑入口、导入导出（CSV/XLSX/JSON），导出文件名使用“页面名称-variable”
- 变量改变弹窗：变量下拉来源于页面变量列表
...
- 组件面板按“布局 / UI 组件 / PC 端组件”分组展示，UI 与 PC 端组件采用父子层级样式。
- 新增布局组件：响应式布局、分栏*1、分栏*2、分栏*4。
- 新增并补齐 PC 端组件：表格、树、下拉菜单、导航菜单、单选框、多选框、级联选择器、标签页、穿梭框、标签、计数器、时间线、图片轮播、网页容器、步骤条、卡片、分页、折叠面板、大数据表格、业务卡片、条形码、滑块、日历、电子签名等。
- 组件卡片预览图标统一为 Element UI 风格的线框蓝色图标，避免空白预览。
- 组件面板内容区支持滚动，避免组件过多被遮挡。

### 变更记录

- 更新组件清单与默认尺寸：`designer/src/manifests/index.js`
- 扩展画布基础渲染（Element Plus 组件与插槽占位）：`designer/src/ui/Canvas/NodeRenderer.vue`
- 调整组件面板分组与图标预览：`designer/src/ui/LeftPanel/ComponentPanel.vue`
- 面板滚动与 Tab 容器布局优化：`designer/src/ui/LeftPanel/MaterialPanel.vue`、`designer/src/assets/styles/main.css`

### 使用说明

1. 打开“组件”面板，搜索或浏览组件分类。
2. 拖拽组件到画布或容器内进行布局。
3. 布局组件用于容器编排，PC 端组件用于具体业务展示。
