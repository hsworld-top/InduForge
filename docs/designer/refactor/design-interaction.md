...
- 页面（Page）是 Designer 的第一导航对象：页面树置顶且独立成区，资源（组件/符号/数据点）为第二层工具。
- 顶部工具栏按任务流组织：页面信息（当前页/脏状态）→ 编辑模式（选择/绘制/排版）→ 视图（缩放/预览）→ 保存发布。
- 右侧面板永不空：未选中元素时默认显示页面设置（Page Inspector），而不是“请选择一个组件”。
...
┌──────────────────────────────────────────────────────────────────────────────┐
│ 顶部工具栏 TopToolbar                                                        │
│ 左：页面名/锁定 | 中：视图+缩放 | 中右：历史/预览 | 右：导出/保存              │
├───────┬──────────────────────────────────────────────┬───────┤
│ ToolRail(L) │ 画布区域 Canvas                         │ ToolRail(R) │
│             │  ┌────────────────────────────────────┐ │             │
│             │  │ Artboard（有效画板：视图尺寸）      │ │             │
│             │  │ - 画板外蒙层                         │ │             │
│             │  │ - 统一视口变换(scale/tx/ty)          │ │             │
...
│             │  （底部：状态栏/缩放/连接状态/诊断入口） │             │
├─────────────┴────────────────────────────────────────┴─────────────┤
│ DockPanel(L/R)：由 ToolRail 打开，固定模式占位/悬浮模式覆盖       │
│ 不提供 Pin/Resize，仅支持固定与悬浮两种显示方式                  │
└───────────────────────────────────────────────────────────────────┘
```
...
```
┌────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ 左区：页面名/锁定  │ 中区：视图切换 + 画板尺寸 + 缩放百分比 │ 中右区：Undo/Redo + 预览 │ 右区：导出 | 保存 │
└────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| 分区        | 功能                  | 说明                                 |
| --------- | ------------------- | ---------------------------------- |
| **左区**     | 页面名 + 锁定 + 脏状态     | 页面名置顶显示，锁定状态可见，脏状态用 ● 标记      |
| **中区**     | 视图切换 + 画板尺寸 + 缩放   | 视图切换为图标组；尺寸显示当前视图宽高；缩放显示百分比 |
| **中右区**   | Undo/Redo + 预览      | 编辑历史与预览入口并列                        |
| **右区**     | 导出 / 保存             | 导出页面与保存入口                         |
...
- 视图类型（编辑器工具栏/预览一致）：
  ...
  - 手机竖向：768px 及以下
  - 手机竖屏：480px 及以下
- 切换视图 = 切换正在编辑的 view schema
- 支持“从 PC 视图克隆到大屏视图”
- 顶部页面信息显示：页面名（当前视图）以免误解

### 3.6 上下文工具显示规则

- 当 当前模式 = 绘图工具（线/矩形/管道/文字等）：上下文工具显示绘图工具组。
- 当 当前模式 = 选择 且 多选数量 >= 2：上下文工具显示对齐/分布/置顶置底。
- 当 当前模式 = 选择 且 选中对象 = 管道/连接线：显示锚点/连接相关工具（可后置）。
- 其它情况：上下文工具组隐藏或收起到 More。

### 3.7 页面切换提示（PageSwitchGuard）

- 若当前页面 dirty=true，用户切换到其他页面时弹窗：
  - 保存并切换
  - 不保存切换
  - 取消
- 若启用自动保存：仅在最近一次保存失败/正在保存且超时/本地与服务器不同步时弹窗（可选优化）。
- 弹窗必须提示：将切换到哪一页。
...
左侧采用 ToolRail + DockPanel 架构（类似 TinyEngine）：

```
左侧 ToolRail + DockPanel
├─ ToolRail(L)：窄条图标导航
│   ├─ 页面树
│   ├─ 大纲树
│   ├─ 物料（组件/绘图/符号库）
...
│   ├─ 预留：AI 对话入口
│   └─ 底部：用户角色管理
└─ DockPanel(L)：由 ToolRail 打开，宽度固定
    ├─ PageNav（页面导航区）
    ├─ Outline（当前页结构）
    ├─ Material（组件/绘图/符号库切换）
    ├─ Data（数据管理与数据点）
    ├─ I18n（多语言资源）
    └─ Script & Vars（脚本/变量）
```
...
- 图标顺序建议：
  ...
  - 预留：AI 对话入口
  - 底部固定：用户角色管理
- 点击图标打开或聚焦对应 DockPanel；再次点击可收起。

### 4.2 DockPanel（左侧停靠抽屉）

- 由 ToolRail 打开，位置固定在左侧区域，宽度固定。
- 固定模式：占据画布空间；非固定模式：悬浮在画布上覆盖显示。
- 不提供 Pin/Resize，仅支持固定与悬浮两种显示方式。
- 允许多面板切换，保持当前上下文。
- 物料面板包含组件与绘图两个切换入口；绘图面板内包含基础绘图与符号库。

### 4.3 页面导航区（PageNav）

PageNav 用于管理工程内的所有页面与页面组：

- 页面/文件夹树（design_pages.parentId 支持文件夹结构）
- 分组展示：系统页 / 业务页
- 页面与分组使用图标区分（页面/文件夹）
- 唯一标记：
  - 登录页：🔐（唯一）
  - 首页：🏠（唯一）
  - 登出落地页：🚪（可选）
- 新建入口：左侧悬浮 + 图标，弹出类型选择（页面/分组/登录页/登出页）与名称输入
- 右键菜单（业务页）：
  - 重命名 / 移动到分组 / 设为首页 / 删除
  - 移动菜单为轻量二级菜单
  - 允许移动到任何分组，但不显示当前所属分组
- 右键菜单（系统页）：仅删除
- 页面与页面组不允许重名；路由路径固定为 /页面名称
- 不支持拖拽排序；切页触发 PageSwitchGuard（见 3.7）

关于登出页的建议：默认“登出”是动作（logout → navigate login）；只有需要展示“已退出页面”时才启用登出落地页。
...
### 5.1 DockPanel（右侧停靠抽屉）

- 由 ToolRail 打开，位置固定在右侧区域，宽度固定。
- 固定模式：占据画布空间；非固定模式：悬浮在画布上覆盖显示。
- 不提供 Pin/Resize，仅支持固定与悬浮两种显示方式。
- 高级面板包含事件绑定与其他扩展配置。
...
RightPanel 根据 SelectionModel 状态分为三种工作模式：

1. Page Inspector（未选中任何元素）
   - 默认显示页面设置，不显示“请选择组件”
   - 点击画布空白处即进入该状态（相当于选中页面/根容器）
2. Element Inspector（选中单个元素）
   - 展示 Tabs：属性 / 样式 / 事件 / 绑定
3. Multi Inspector（多选）
   - 显示共同属性与批量样式
   - 混合值显示 --

### 5.2.1 Page Inspector 占位 UI
...
│ - 设为首页 [设为]   - 设为登录页 [设为]                       │
│ 画布：                                                      │
│ - 画板尺寸 1920×1080（来自当前视图）                          │
│ - 背景/网格/吸附（开关）                                     │
│ 诊断：                                                      │
│ - 本页数据点：正常 X / 失效 Y（可点击展开列表）               │
└──────────────────────────────────────────────────────────────┘
```

右侧面板为选中元素（Canvas 图形或 DOM 组件）提供统一的配置界面。

### 5.3 属性面板（PropertyPanel）
...
### 6.0 Empty State 类型区分

- EmptyPageHint（页面空）：当页面 Canvas + DOM 都为空时显示
  - 引导按钮：添加容器（Flex/Free）、开始绘制（矩形/管道）、从模板创建（可选）
- EmptyContainerPlaceholder（容器空）：当容器 children=[] 时显示虚线占位符
  - 仅设计态渲染，运行态不渲染
- InspectorEmpty（右侧空）：不再出现（未选中显示 Page Inspector）
...
- 占位符为设计态渲染层产物（DesignRenderer Overlay），不得写入 schema
- 占位符需要参与 Drop 命中测试（dragenter/dragleave/dragover），以便实现“Drop Here”高亮
- 占位符仅在设计态显示，运行态不渲染
- 占位符使用虚线边框 + 浅灰背景
- 最小高度 48px
- 显示提示文字
...
class AutoSaveManager {
  ...
  start(): void {
    ...
    window.addEventListener("beforeunload", (e) => {
      if (this.isDirty) {
        // 仅保证 IndexedDB 本地落盘，服务端保存由后续自动保存/恢复流程处理
