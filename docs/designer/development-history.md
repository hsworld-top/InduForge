...
- 组件面板重构与扩展：布局组件调整为响应式布局/分栏*1/分栏*2/分栏*4；UI 组件按父子层级展示，PC 端组件补齐（穿梭框、标签、计数器、时间线、图片轮播、网页容器、步骤条、卡片、分页、折叠面板、大数据表格、业务卡片、条形码、滑块、日历、电子签名等）；预览图标统一为 Element UI 风格线框蓝色；面板内容区支持滚动；画布侧补齐 Element Plus 基础渲染与插槽占位。
