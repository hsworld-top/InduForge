...
- 高级面板调整：事件与绑定融合，组件选中展示事件，画布空白/网格展示页面绑定；绑定新增定时器/变量改变配置与统一脚本编辑器；变量页签支持页面变量管理、导入导出与编辑入口
...
- 页面属性修改：页面属性与画布属性联动优化。点击画布时右侧切换为页面属性面板；将画布样式配置入口移动到页面设置 > 画布设置，新增样式配置弹窗并支持模板插入，模板按 domId 作用域输出；修复样式模板换行与文案显示异常。
- 页面属性优化：画布设置新增“自适应”开关，预览态支持画布 100% 填充或按实际尺寸展示；移除页面属性中的“适配模式”；背景设置下沉到画布并支持颜色/图片/渐变且与网格叠加；根画布下 ElContainer 默认 100% 铺满并禁用拖拽/缩放；修复标尺遮挡导致 Header 区域显示不全。
- ElLayout：修复右侧属性以及行间距内部区域自适应
- 预览运行时接入页面生命周期（创建/关闭/定时器）与页面变量 `$vars` 支持，补齐预览脚本上下文。
- 预览入口传入 `pageLifecycle` 与 `pageVariables`，并修复预览页面文案乱码问题。
- 保存页面时写入页面变量到 `payload.vars.pages`，加载页面时合并 `payload.vars`，避免预览返回后变量丢失。
