...
本文档定义工程不可变 Release 的 Manifest。该 Manifest 是 `dev_core`、site-controller、
NodeAgent、Release Loader、`project-nginx`、`runtime-api` 和 `runtime-engine` 的共同输入。
...
```json
{
  ...
  "compatibility": {
    "minSiteControllerVersion": "1.0.0",
...
| `compatibility`       | 是   | 控制器、Runtime 与站点能力兼容要求 |
...
资源绑定到目标站点的实际资源。晋级生产时创建新的 Binding revision 和 DeploymentRun，不重新
构建或修改 Release。
...
- 目标 Site、控制器和 Runtime 不满足 compatibility 时必须阻止部署。
