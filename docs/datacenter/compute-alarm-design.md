...
1. 工程用户与角色配置入口统一放在“工程卡片”的 `成员与权限`。
2. 设计中心与数据中心共用同一套工程级 RBAC，不各自维护独立角色配置入口。
3. 设计中心与数据中心只消费权限能力点（capabilities），不维护角色主数据。
4. 角色调整后应即时对设计中心与数据中心生效，避免权限口径不一致。
...
16. 工程用户角色配置入口统一上移到工程卡片，设计中心与数据中心共用。
