...
工程运行系统部署在生产站点，承载发布后工程的前端静态应用、Runtime API、数据处理、报警、
计算、存储和动作执行。运行用户直接访问站点为工程分配的独立地址，中心平台不代理运行请求。
...
站点共享 K3s、NATS JetStream、Ingress Controller、StorageClass、监控日志和 K3s 外的
Site NodeAgent。工程资源始终随不可变 Release 交付，站点公共服务不解析工程前端内容。
...
| Project  | 独立 K3s Namespace 和 NATS Account                     |
| Project  | 独立 ServiceAccount、Secret、ConfigMap、PVC 和数据空间 |
| Release  | 独立不可变工程版本                                     |
...
工程前端是 `project-nginx` 托管的静态文件，不运行常驻 Node 服务。
...
- 小站点允许单节点 K3s 和单节点 JetStream，重要站点使用高可用部署 Profile。
