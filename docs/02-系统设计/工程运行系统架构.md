...
`project-nginx` 是工程 Release 的默认 Web 服务器；站点只有在需要 Kubernetes Ingress 的动态路由、
跨工程 TLS/域名、证书自动化或统一入口策略时才引入 Traefik（或经评审选定的 Ingress Controller）。
Ingress Controller 不替代 `project-nginx` 的静态托管和 API/WebSocket 代理。当前 K3s 发布路径尚未
实现，实际可验收范围见[运维体系实现与验收](../06-运维与安全/运维体系实现与验收.md)。
