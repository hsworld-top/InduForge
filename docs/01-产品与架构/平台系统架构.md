...
- 每个工程使用独立 K3s Namespace、NATS Account、配置、密钥和数据空间。
...
由中心节点管理能力和各站点 Site NodeAgent 组成。
...
Site NodeAgent 运行在 K3s 外部，Linux 使用 systemd，Windows 使用 Windows Service。
...
```mermaid
graph LR
    Model["建模与页面设计"] --> Validate["契约检查"]
    Validate --> Build["构建不可变 Release"]
    Build --> S3["中心 S3"]
    S3 --> Publish["创建发布期望状态"]
    Publish --> Agent["Site NodeAgent"]
    Agent --> K3s["K3s 工程 Namespace"]
    Agent --> Native["原生采集进程"]
    K3s --> Running["工程运行"]
    Native --> Running
```
...
- 页面与运行资源由工程 Namespace 内的 Release Loader 下载到工程 PVC。
- 采集配置由采集节点 NodeAgent 下载到该工程的本地版本目录。
- Collector 程序和驱动属于节点能力包，不随每次工程 Release 重复交付。

## 9. 架构约束

- 中心不得成为站点实时运行链路的一部分。
- Site NodeAgent 不解释页面、数据点、报警和采集地址语义。
