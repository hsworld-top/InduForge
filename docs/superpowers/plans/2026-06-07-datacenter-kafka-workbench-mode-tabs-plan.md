...
```ts
const outputMode = props.mode === 'edit' ? props.mapping?.outputMode || form.outputMode : form.outputMode
```
...
```vue
<WorkbenchStatusPill :label="mapping.rawOutputScope === 'full_message' ? '完整消息' : '消息体'" tone="info" />
...
const parseEditorFields = () => {
  ...
  try {
    ...
    candidateRows.value = inferKafkaSampleFields([{ value: parsed }], fields.value.map((field) => field.valuePath))
...
if (firstSample) {
  ...
  candidateRows.value = inferKafkaSampleFields(preview.samples || [], fields.value.map((field) => field.valuePath))
}
```
