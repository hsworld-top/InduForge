...
| 方法                           | 语义                                         | 开发态                                      |
| ------------------------------ | -------------------------------------------- | ------------------------------------------- |
| `get(options?)`                | 获取业务值样本；按需型数据点可能触发一次来源执行 | 读取已预取快照                           |
...
`get()`、`read()` 与 `peek()` 均返回 `SDKResult<DataPointSample<T>>`，业务值统一从 `result.data.value` 读取。三者的差异是是否触发来源以及读取语义，而不是返回结构。
...
试运行按变量名提供结构化快照。只覆盖当前试运行的数据点值、质量和时间，不修改真实数据点：
...
即使数据点业务值本身是对象，也必须放在 `value` 内，避免把对象值误认为快照属性。
