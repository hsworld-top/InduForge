...
- “报警”按独立报警项分页集中管理，一行对应一个数据点上的一条报警；同一点可同时拥有越限、变化率、偏差、离线等报警。多点创建生成多条独立项，表格支持当前选择或全部筛选结果的批量启停、目录、通知、兼容条件替换和删除。目录默认收起且只用于筛选整理；数据点页面只读展示全部有效报警并跳转报警单元。普通报警支持 Excel 模板、导出、预览和事务导入，组合报警仍在专用工作台维护。
- “报警历史”只配置节点内置报警库的保存开关、保留期限和通知投递记录开关，不展示存储目标，也不提供历史查询。
- 计算触发支持手动、周期、每日、每周和数据点变化；每日/每周明确 IANA 时区并预览未来五次执行。定时与点变属于节点运行配置，开发态不自动执行。
