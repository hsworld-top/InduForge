...
“节点与工程部署”负责节点接入、审批、心跳、运行环境和部署管理。工程卡片根据服务端部署摘要
显示发布、更新开发版、发布新版本、查看进度或处理异常等动作；发布弹窗选择开发版或正式版本、
运行环境和引擎分布，并提交部署请求。多部署场景跳转部署管理，避免替用户猜测目标。
界面入口与代码实现不代表目标环境已验收；实际交付范围、安装包与验收见
...
```text
{projectId}:ai
{projectId}:2d:{sceneId}
{projectId}:3d:{sceneId}
```

分别对应工程 AI 页面开发工作台和按场景区分的 2D、3D 编辑器。相同工程、工具与场景的重复
打开请求必须激活已有标签；同一工程的不同场景可以同时打开。

AI 标签标题包含工程名称，例如“产线监控 · AI开发”；场景标签使用场景名，例如“主画面 · 2D”。
标签切换时隐藏但不销毁实例，只有用户关闭标签、工程被删除或权限失效时才销毁。
...
2. 校验场景 ID、场景名称和目标工具，并由控制面校验工程及场景访问权限。
3. 按工程、工具和场景复用或创建稳定标签。
4. 从控制面获取该场景的短期编辑会话与受控访问 URL。
5. 激活标签并保持其他未关闭实例状态。
...
`frontend-linux` 模式时，仍按根 `.env` 的 `VITE_API_URL`、`VITE_DATA_SERVICE_URL` 分别连接本机
控制面和数据服务。
