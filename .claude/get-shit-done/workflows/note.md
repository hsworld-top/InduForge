...
- **Global scope**: `/mnt/d/SVNCode/indu-forge/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/mnt/d/SVNCode/indu-forge/.claude/notes/`)
...
2. Glob `/mnt/d/SVNCode/indu-forge/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/mnt/d/SVNCode/indu-forge/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/mnt/d/SVNCode/indu-forge/.claude/notes/` — works in any directory
