...
<template>
@D:/SVNCode/indu-forge/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "D:/SVNCode/indu-forge/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(node "D:/SVNCode/indu-forge/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "D:/SVNCode/indu-forge/.claude/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "D:/SVNCode/indu-forge/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @D:/SVNCode/indu-forge/.claude/get-shit-done/workflows/diagnose-issues.md
