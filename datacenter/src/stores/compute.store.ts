...
export const useComputeStore = defineStore('compute', () => {
  ...
  async function saveFolder(projectId: string, folderId: string, data: Partial<ComputeFolderSave>) {
    saving.value = true
    try {
  ...
  async function saveUnit(projectId: string, id: string, data: Partial<ComputeUnitSave>) {
    saving.value = true
    try {
      const unit = await updateComputeUnit(projectId, id, data)
      editing.value = unit
      list.value = list.value.map((item) => (item.id === unit.id ? { ...item, ...unit } : item))
      return unit
    } finally {
      saving.value = false
    }
