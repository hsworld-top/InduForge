...
export const KafkaTopicMappingSchema = z
  .object({
    ...
    consumerGroup: z.string().default('').optional(),
    ...
    startOffset: z.number().nullable().default(null).optional(),
...
export const KafkaFieldSchema = z
  .object({
    ...
    groupId: IdSchema.nullable().optional(),
    name: z.string(),
    valuePath: z.array(z.union([z.string(), z.number().int().nonnegative()])),
    keyPath: z.array(z.union([z.string(), z.number().int().nonnegative()])).default([]),
    dataType: z.string(),
