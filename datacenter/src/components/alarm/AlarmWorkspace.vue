...
const activeTargetSignature = computed(() => {
  ...
  return draft.targets.map((target) => `${target.datapointId || ''}:${target.path || ''}`).join('|')
})
...
const cleanParams = (params: Record<string, string>) =>
  Object.fromEntries(Object.entries(params).filter(([, value]) => value.trim() !== ''))
...
async function confirmDirtyClose(name: string) {
  try {
    await ElMessageBox.confirm(`报警策略「${name}」有未保存修改。`, '关闭标签', {
...
const hasCoverageConflicts = (draft: AlarmPolicyDraft) =>
  ...
  draft.targets.some(
    (target) => (coverageByTargetId.value[coverageKey(target)]?.policies.length || 0) > 0,
  )
...
const validateDraft = (draft: AlarmPolicyDraft, requireReady = true) => {
  ...
  if (draft.mode === 'per_target' && draft.targets.length === 0) {
...
const toggleEnabled = async () => {
  ...
  const policy = await alarmStore.setPolicyEnabled(props.projectId, policyId, enable)
  setDraft(policyId, toAlarmPolicyDraft(policy))
  await reloadList()
}
...
const countAlarmChildGroups = (group: AlarmPolicyGroupNode): number =>
  group.children.reduce((total, child) => total + 1 + countAlarmChildGroups(child), 0)
...
const batchDelete = async () => {
  ...
  if (
    ...
    !alarmStore.tree.policies.some((item) => item.id === selectedPolicyId.value)
  ) {
    const nextDrafts = { ...drafts.value }
