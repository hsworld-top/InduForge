  ...
  <div class="postgres-query-editor flex flex-col h-full">
    ...
    <div class="flex items-center justify-between mb-3 pb-3 border-b border-gray-200 dark:border-gray-700">
        ...
        >
          ...
          >
            <div class="flex items-center justify-between w-full">
              ...
              <span class="text-xs text-gray-400 ml-2 flex-shrink-0">{{ conn.relationalConfig?.database }}</span>
            </div>
          </el-option>
        </el-select>
        <el-select
      ...
      <div class="flex items-center space-x-2">
        ...
        <el-button type="primary" size="small" @click="handleExecute" :loading="localTab.executing">
    ...
    <div class="mb-2 flex items-center justify-between">
      <div class="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
        ...
        <span>提示：使用 <code class="px-1 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">$1, $2, ...</code> 作为参数占位符，例如：<code class="px-1 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">WHERE id = $1</code></span>
      </div>
    </div>
    ...
    >
      <div class="flex items-center justify-between mb-2">
        <div class="flex items-center space-x-2">
          ...
          <el-tag size="small" type="info">{{ localTab.parameters.length }} 个参数</el-tag>
        </div>
        <el-button size="small" text @click="localTab.parametersExpanded = !localTab.parametersExpanded">
          <component 
            :is="localTab.parametersExpanded ? IconTablerChevronDown : IconTablerChevronRight" 
            class="mr-1 w-4 h-4"
          />
    ...
    <!-- 查询结果 -->
    <div v-if="localTab.result" class="flex-1 flex flex-col border-t border-gray-200 dark:border-gray-700 pt-4 min-h-0">
      <div class="flex items-center justify-between mb-2 flex-shrink-0">
        <div class="text-sm text-gray-600 dark:text-gray-400">
          查询结果 ({{ localTab.result.rowCount }} 行，耗时 {{ localTab.result.executionTime }}ms)
        </div>
      ...
      <div class="flex-1 flex flex-col border border-gray-200 dark:border-gray-700 rounded min-h-0">
        ...
        <div class="flex items-center justify-between px-4 py-2 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex-shrink-0">
          <div class="text-sm text-gray-600 dark:text-gray-400">
            显示第 {{ (localTab.resultPage - 1) * localTab.resultPageSize + 1 }} - 
            {{ Math.min(localTab.resultPage * localTab.resultPageSize, localTab.result.rowCount) }} 条，
            共 {{ localTab.result.rowCount }} 条
          </div>
          <el-pagination
