<template>
  <DcDialog v-model="visible" title="导入 S7 地址表" width="860px">
    ...
    <el-table class="s7-import-dialog__table" :data="rows" height="260" empty-text="粘贴表格文本后预览">
      <el-table-column label="状态" width="76"><template #default="{ row }"><el-tag size="small" :type="row.issue ? 'warning' : 'success'">{{ row.issue ? '检查' : '可导入' }}</el-tag></template></el-table-column>
    ...
    <template #footer>
      ...
      <el-button type="primary" :disabled="rows.length === 0" :loading="loading" @click="$emit('submit', rows)">导入 {{ rows.length }} 个变量</el-button>
    </template>
  </DcDialog>
</template>
...
const emit = defineEmits<{ (event: 'update:modelValue', value: boolean): void; (event: 'submit', rows: Array<Record<string, unknown>>): void }>()
