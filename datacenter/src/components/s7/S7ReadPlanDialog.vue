<template>
  <DcDialog v-model="visible" title="S7 读取计划估算" width="860px">
    <div class="s7-read-plan-dialog__metrics">
      <span
        ><b>{{ estimate.variableCount }}</b
        >变量</span
      >
      <span
        ><b>{{ estimate.blockCount }}</b
        >读取块</span
      >
      <span
        ><b>{{ estimate.totalReadBytes }}</b
        >bytes</span
      >
      <span
        ><b>{{ estimate.readsPerSecond.toFixed(2) }}</b
        >reads/s</span
      >
    </div>
    <el-table :data="estimate.plans" height="360" empty-text="暂无读取计划">
      <el-table-column prop="area" label="区域" width="76" />
      <el-table-column label="DB号" width="76"
        ><template #default="{ row }">{{ row.dbNumber ?? '-' }}</template></el-table-column
      >
      <el-table-column label="读取范围" min-width="160" show-overflow-tooltip
        ><template #default="{ row }">{{
          row.displayRange || `${row.startByte}-${row.endByte}`
        }}</template></el-table-column
      >
      <el-table-column prop="readLength" label="字节" width="80" />
      <el-table-column prop="variableCount" label="变量数" width="90" />
      <el-table-column label="周期" width="96"
        ><template #default="{ row }">{{ row.pollIntervalMs }}ms</template></el-table-column
      >
    </el-table>
