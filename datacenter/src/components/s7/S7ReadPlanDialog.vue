<template>
  <el-dialog :model-value="modelValue" title="S7 读取计划估算" width="820px" @close="$emit('update:modelValue', false)">
    ...
    <el-table :data="estimate.plans" height="360" empty-text="暂无读取计划">
      ...
      <el-table-column label="读取范围" min-width="140"><template #default="{ row }">{{ row.displayRange || `${row.startByte}-${row.endByte}` }}</template></el-table-column>
...
defineProps<{ modelValue: boolean; estimate: S7ReadPlanEstimate }>()
