<template>
  <section class="s7-workbench">
    <aside class="s7-workbench__side">
      ...
      <S7GroupTree :groups="groups" :variables="variables" :selected-group-id="selectedGroupId" @select="selectGroup" @create="openCreateGroup" @edit="openEditGroup" @delete="removeGroup" />
    </aside>
    <main class="s7-workbench__main">
      ...
      <S7VariableTable :variables="filteredVariables" :loading="loading" :selected-variable-id="selectedVariableId" @select="selectVariable" @edit="openEditVariable" @delete="removeVariable" />
    </main>
    <S7InspectorPanel :profile="profile" :group="currentGroup" :variable="selectedVariable" :variables="variables" :issues="scopedValidationIssues" :estimate="readPlanEstimate" />

    <S7ProfileDialog v-model="profileVisible" :profile="profile" :loading="saving" @submit="saveProfile" />
    <S7GroupDialog v-model="groupVisible" :mode="groupMode" :groups="groups" :group-value="editingGroup" :loading="saving" @submit="saveGroup" />
    <S7VariableDialog v-model="variableVisible" :mode="variableMode" :groups="groups" :variable="editingVariable" :default-group-id="selectedGroupId" :loading="saving" @submit="saveVariable" />
    <S7ImportDialog v-model="importVisible" :loading="saving" @submit="importVariables" />
    <S7PreviewDialog v-model="previewVisible" :values="previewValues" :diagnostics="previewDiagnostics" />
    <S7ValidationDrawer v-model="validationVisible" :issues="scopedValidationIssues" :scope-label="validationScopeLabel" @locate="locateVariable" />
...
import type { S7Profile, S7ReadPlanEstimate, S7ReadValue, S7ValidationIssue, S7Variable, S7VariableGroup } from '@/components/s7/types'
...
type AccessSourceConnection = { id: string; name?: string; type?: string; status?: string; config?: Record<string, any> }
const props = defineProps<{ connection: AccessSourceConnection; projectId: string }>()
defineEmits<{ (event: 'back'): void }>()

const emptyEstimate = (): S7ReadPlanEstimate => ({ variableCount: 0, blockCount: 0, totalReadBytes: 0, readsPerSecond: 0, estimatedCycleMs: 0, largestBlockBytes: 0, fragmentedGroupCount: 0, plans: [], diagnostics: [] })
...
const sourceMetaRows = computed(() => [
  ...
  { label: '端点', value: profile.value ? `${profile.value.host}:${profile.value.port}` : `${config.value.host || '未配置 host'}:${config.value.port || 102}` },
...
const filteredVariables = computed(() => {
  ...
  return variables.value.filter((item) => {
    ...
    const matched = !text || [item.name, item.code, item.normalizedAddress, item.addressText, item.datapointPath || ''].some((value) => String(value || '').toLowerCase().includes(text))
    return inGroup && matched
  })
})
const validationScopeLabel = computed(() => selectedVariable.value ? `当前变量：${selectedVariable.value.name}` : currentGroup.value ? `当前分组：${currentGroup.value.name}` : '全部变量')
const scopedValidationIssues = computed(() => {
  if (selectedVariableId.value) return validationIssues.value.filter((item) => item.variableId === selectedVariableId.value)
  if (selectedGroupId.value) {
    const ids = new Set(variables.value.filter((item) => item.groupId === selectedGroupId.value).map((item) => item.id))
...
const reloadEstimate = async () => {
  const response = await dataAPI.getS7ReadPlanEstimate(props.projectId, props.connection.id, { groupId: selectedGroupId.value || undefined })
...
const selectGroup = async (groupId: string) => { selectedGroupId.value = groupId; selectedVariableId.value = ''; await reloadEstimate() }
const selectVariable = (variable: S7Variable) => { selectedVariableId.value = variable.id }
const openCreateGroup = () => { editingGroup.value = null; groupMode.value = 'create'; groupVisible.value = true }
const openEditGroup = (group: S7VariableGroup) => { editingGroup.value = group; groupMode.value = 'edit'; groupVisible.value = true }
const saveProfile = async (payload: Record<string, unknown>) => {
  ...
  try {
    ...
    await loadProfile()
    await reloadEstimate()
  } catch (error) { ElMessage.error(getApiErrorMessage(error, '保存 PLC 档案失败')) } finally { saving.value = false }
}
const saveGroup = async (payload: Record<string, unknown>) => {
  saving.value = true
  try {
    if (groupMode.value === 'edit' && editingGroup.value) await dataAPI.updateS7VariableGroup(props.projectId, props.connection.id, editingGroup.value.id, { ...payload, hasParentId: true })
    else await dataAPI.createS7VariableGroup(props.projectId, props.connection.id, payload)
    groupVisible.value = false
    await reloadAll()
  } catch (error) { ElMessage.error(getApiErrorMessage(error, '保存变量组失败')) } finally { saving.value = false }
}
...
const openCreateVariable = () => { editingVariable.value = null; variableMode.value = 'create'; variableVisible.value = true }
const openEditVariable = (variable: S7Variable) => { editingVariable.value = variable; variableMode.value = 'edit'; variableVisible.value = true }
const saveVariable = async (payload: Record<string, unknown>) => {
  saving.value = true
  try {
    const saved = variableMode.value === 'edit' && editingVariable.value
  ...
  } catch (error) { ElMessage.error(getApiErrorMessage(error, '保存变量失败')) } finally { saving.value = false }
}
...
const importVariables = async (rows: Array<Record<string, unknown>>) => {
  saving.value = true
  try {
    await dataAPI.batchImportS7Variables(props.projectId, props.connection.id, { groupId: selectedGroupId.value || null, variables: rows })
    importVisible.value = false
    await reloadAll()
  } catch (error) { ElMessage.error(getApiErrorMessage(error, '导入变量失败')) } finally { saving.value = false }
}
const toggleSession = () => { if (session.connected.value) void session.disconnect(); else void session.connect() }
const applyReadValues = (values: S7ReadValue[]) => {
  ...
  variables.value = variables.value.map((variable) => {
    ...
    return next ? { ...variable, lastValue: next.value, quality: next.quality, lastUpdatedAt: next.timestamp } : variable
  })
}
const readCurrentScope = async () => {
  if (!session.connected.value || !session.sessionId.value) return ElMessage.warning('请先连接 S7 开发态会话')
  const response = await dataAPI.readS7DevSession(props.projectId, props.connection.id, session.sessionId.value, { groupId: selectedGroupId.value || null })
  const values = unwrapData(response).values || []
  applyReadValues(values)
}
const openPreview = async () => {
  if (!session.connected.value || !session.sessionId.value) return ElMessage.warning('请先连接 S7 开发态会话')
  const response = await dataAPI.pollS7DevSession(props.projectId, props.connection.id, session.sessionId.value, { groupId: selectedGroupId.value || null })
...
const openReadPlan = async () => { await reloadEstimate(); readPlanVisible.value = true }
