      ...
      <div class="access-source-detail__body">
        ...
        <template v-else-if="activeTab === 'mapping'">
          <section class="access-source-detail__section">
            ...
            <div v-if="detailLoading" class="access-source-detail__empty-line">
              加载映射中...
            </div>
            <template v-else-if="isSqlConnection">
              ...
              >
                <div>
                  ...
                  <span>{{
                    item.point?.path || "保存后会自动生成 db.query 数据点"
                  }}</span>
                </div>
              ...
              <div
                v-if="queryMappings.length === 0"
                class="access-source-detail__empty-line"
              >
        ...
        <template v-else-if="activeTab === 'preview'">
          <section class="access-source-detail__section">
            ...
            <div class="access-source-detail__preview-box">
              ...
              <el-button size="small" @click="$emit('open', connection)">
                进入工作台
              </el-button>
            </div>
          </section>
        ...
        <template v-else-if="activeTab === 'datapoints'">
          <section class="access-source-detail__section">
            ...
            <template v-else>
              ...
              <div
                v-if="detailDataPoints.length === 0"
                class="access-source-detail__empty-line"
              >
                {{ datapointEmptyText }}
              </div>
...
const isSqlConnection = computed(() => {
  ...
  return (
    type === "relational" ||
    ["mysql", "postgresql", "sqlserver", "tdengine"].includes(type)
  );
});
...
const endpointText = computed(() => {
  ...
  if (connection.type === "mqtt") {
    ...
    const endpoint = [config.brokerUrl || config.host, config.port]
      ...
      .join(":");
    return (
      [endpoint, config.topic || config.defaultTopic]
        .filter(Boolean)
        .join(" / ") || "未配置"
    );
  }
  ...
  if (connection.type === "kafka") {
    return (
      [config.brokers, config.topic].filter(Boolean).join(" / ") || "未配置"
    );
  }
  ...
  if (connection.type === "tdengine") {
    return (
      [config.dsn, config.database].filter(Boolean).join(" / ") || "未配置"
    );
  }
...
const datapointCountText = computed(() => {
  ...
  return typeof fallbackDatapointCount.value === "number"
    ? `${fallbackDatapointCount.value}`
    : "0";
});
...
const sourceModeText = computed(() => {
  ...
  if (props.connection?.type === "mqtt")
    return "订阅 Tag 自动生成 mqtt.tag 数据点";
...
const capabilityText = computed(() => {
  ...
  if (
    ["kafka", "http", "websocket", "redis"].includes(
...
const openHint = computed(() => {
  if (isSqlConnection.value)
    return "查看表、执行 SQL、保存查询并自动形成数据点。";
  if (props.connection?.type === "mqtt")
    return "维护订阅、查看消息、生成和监控 Tag。";
...
const datapointEmptyText = computed(() => {
  if (isSqlConnection.value)
    return "保存 SQL 查询后，后端会自动同步 db.query 数据点。";
  if (props.connection?.type === "mqtt")
    return "创建 MQTT Tag 后会在数据点列表中出现。";
  return "当前接入源暂未生成数据点。";
});
...
const loadDetailResources = async () => {
  ...
  try {
    if (isSqlConnection.value) {
      ...
      const sourceIds = detailQueries.value
        .map((query) => query.id)
        .filter(Boolean);
...
const resolvePointStatus = (point: any) =>
  point.status === "invalid" ? "失效" : "活跃";
