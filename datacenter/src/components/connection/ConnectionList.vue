<template>
  <div class="connection-list w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
    <div class="p-4 flex-1 overflow-y-auto min-h-0">
      <div class="space-y-2">
              ...
              <div class="mt-2 space-y-2" @dblclick.stop>
                ...
                <div class="space-y-1 pb-2 border-b border-gray-200 dark:border-gray-700">
                  ...
                  >
                    ...
                    >
                      <component 
                        :is="getConnectionState(connection.id).queriesExpanded ? IconTablerChevronDown : IconTablerChevronRight" 
                  ...
                  <div v-if="getConnectionState(connection.id).loadingQueries" class="pl-6 text-xs text-gray-500">
                    加载查询列表...
                  </div>
                  <template v-else-if="getConnectionState(connection.id).queriesExpanded">
                    <div v-if="getConnectionState(connection.id).queries.length === 0" class="pl-6 text-xs text-gray-500">
                      暂无查询
                    </div>
                    <ul v-else class="space-y-0.5 pl-6 border-l-2 border-blue-300 dark:border-blue-700">
                      <li
                        v-for="query in getConnectionState(connection.id).queries"
                        :key="query.id"
                ...
                <div class="space-y-1 pt-1">
                  ...
                  >
                    ...
                    >
                      <component 
                        :is="getConnectionState(connection.id).tablesExpanded ? IconTablerChevronDown : IconTablerChevronRight" 
                    ...
                    <div class="flex items-center space-x-1">
                      ...
                      <el-tooltip content="查看表列表" placement="top">
                        ...
                        >
                          <IconTablerTable class="w-3 h-3 text-green-600 dark:text-green-400" />
                        </button>
                      </el-tooltip>
                      <span v-if="!getConnectionState(connection.id).loading" class="text-green-600 dark:text-green-400">
                        {{ getConnectionState(connection.id).tables.length }}
                      </span>
                    </div>
                  </div>
                  <div v-if="getConnectionState(connection.id).loading" class="pl-6 text-xs text-gray-500">
                    加载表列表...
                  </div>
                  <template v-else-if="getConnectionState(connection.id).tablesExpanded">
                    <div v-if="getConnectionState(connection.id).tables.length === 0" class="pl-6 text-xs text-gray-500">
                      暂无表
                    </div>
                    <ul v-else class="space-y-0.5 pl-6 border-l-2 border-green-300 dark:border-green-700">
                      <li
                        v-for="table in getConnectionState(connection.id).tables"
                        :key="table.name"
                      ...
                      >
                        ...
                        <span class="text-gray-400 text-[10px]">{{ table.rows }}</span>
                      </li>
                    </ul>
                  </template>
                </div>
        ...
        <div v-if="connections.length === 0" class="text-center py-8 text-gray-500">
          ...
          <el-button type="primary" size="small" @click="handleCreate" class="mt-2">
            创建连接
          </el-button>
        </div>
      </div>
...
const toggleSection = async (connectionId, section) => {
  ...
  if (section === 'tables') {
    ...
    // 如果展开且还没加载查询列表，则加载
    if (state.queriesExpanded && state.queries.length === 0 && !state.loadingQueries) {
...
const loadTables = async (connectionId) => {
  ...
  try {
    const response = await dataAPI.getConnectionTables(props.projectId, connectionId)
    if (response.success) {
...
const loadQueries = async (connectionId) => {
  ...
  try {
    const response = await dataAPI.getQueries(props.projectId, { connectionId })
    if (response.success) {
      state.queries = response.data.queries || []
    }
  } catch (error) {
    ElMessage.error('加载查询列表失败：' + (error.response?.data?.message || error.message))
  } finally {
