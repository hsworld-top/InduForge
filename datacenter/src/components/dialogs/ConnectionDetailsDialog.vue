    ...
    <el-descriptions :column="1" border v-if="connection">
      ...
      <template v-if="connection.type === 'mqtt' && connection.mqttConfig">
        ...
        <el-descriptions-item label="客户端 ID">{{
          connection.mqttConfig.clientId || "自动生成"
        }}</el-descriptions-item>
        <el-descriptions-item label="用户名">{{
        ...
        <el-descriptions-item label="保持连接"
          >{{ connection.mqttConfig.keepalive }}秒</el-descriptions-item
        >
        <el-descriptions-item label="清除会话">{{
          connection.mqttConfig.cleanSession ? "是" : "否"
        }}</el-descriptions-item>
      </template>
