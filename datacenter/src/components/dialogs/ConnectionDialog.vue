  ...
  >
    ...
    <el-form v-if="mode === 'create'" label-width="120px" class="mb-4">
      <el-form-item label="连接类型">
        <el-select v-model="connectionType" placeholder="选择连接类型" class="w-full" @change="handleTypeChange">
          <el-option label="关系数据库" value="relational" />
        </el-select>
      </el-form-item>

      <el-form-item v-if="connectionType === 'relational'" label="数据库类型">
        <el-select v-model="dbType" placeholder="选择数据库类型" class="w-full" @change="handleDbTypeChange">
          <el-option
