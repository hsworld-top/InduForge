...
export function buildComputeFolderTree(folders: ComputeFolder[], units: ComputeUnit[]) {
  ...
  const rootFolders = [...folderMap.values()].filter((node) => !childIdSet.has(node.id))
...
export function filterComputeTree(
  ...
  const matchesFolder = (folder: ComputeFolderTreeNode) => folder.name.toLowerCase().includes(lower)

  const filterFolder = (folder: ComputeFolderTreeNode): ComputeFolderTreeNode | null => {
