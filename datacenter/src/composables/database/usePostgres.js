...
export function usePostgres(projectId, connectionId) {
  ...
  const loadTables = async () => {
    ...
    try {
      ...
      ElMessage({
        type: 'error',
        message: '加载表列表失败：' + (error.response?.data?.message || error.message),
        offset: 60,
        duration: 5000,
        showClose: true
      })
  ...
  const formatSql = (sql) => {
    ...
    let formatted = sql
      .replace(/\s+/g, ' ')
      .replace(/\s*,\s*/g, ',\n  ')
      .replace(/\s+(FROM|WHERE|GROUP BY|ORDER BY|LIMIT|OFFSET|HAVING|JOIN|LEFT JOIN|RIGHT JOIN|INNER JOIN)/gi, '\n$1')
