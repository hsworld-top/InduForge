...
export function createMqttSocketSharedRegistry(options = {}) {
  ...
  const resolveSocketUrl = () => {
    const raw = typeof getApiUrl === 'function' ? getApiUrl() : getApiUrl || DEFAULT_SOCKET_URL
  ...
  return {
    ...
    subscribeSubscription(key, subscriptionId) {
      ...
      const nextCount = (entry.subscriptionRefs.get(normalizedSubscriptionId) || 0) + 1
    ...
    unsubscribeSubscription(key, subscriptionId) {
      ...
      const currentCount = entry.subscriptionRefs.get(normalizedSubscriptionId) || 0
      if (currentCount <= 1) {
