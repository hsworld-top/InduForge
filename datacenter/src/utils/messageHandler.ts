...
import { applyThemeToDocument, isTrustedHostMessage } from '../runtime/host-bootstrap'
...
class MessageHandler {
  ...
  async handleMessage(event) {
    ...
    if (data.type === 'REQUEST' && data.requestId) {
      ...
      try {
        const result = await this.handleRequest(data.action, data.payload, replyTarget)
        this.sendMessage(replyTarget, {
          type: 'RESPONSE',
   ...
   * @returns {Promise<Object>}
   */
  async subscribe({ subscriptionId, projectId, queryId, parameters, interval }, replyTarget) {
    if (!replyTarget) {
