...
test('locale 归一化与翻译输出', () => {
  ...
  assert.equal(getDatacenterRouteTitle('datacenter-debug'), 'Data Center Debug')
...
test('标签标题会按当前语言实时重算', () => {
  assert.equal(
    resolveDatacenterTabLabel({ labelPrefix: '连接A', labelKey: 'tabs.tableList' }, (key) =>
      key === 'tabs.tableList' ? 'Tables' : key,
    ),
    '连接A - Tables',
  )
})
...
test('宿主 LOCALE_UPDATE 会更新响应式 locale', () => {
  ...
  const handler = createRuntimeMessageHandler({
    ...
    isTrustedHostMessage: (event, context) =>
      context.trustedOrigins.has(event.origin) && context.trustedSources.has(event.source),
