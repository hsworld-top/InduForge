...
test("buildIdeRestoreUrl 会复用 handoff，没有 handoff 时回 IDE 首页", () => {
  ...
  assert.equal(
    buildIdeRestoreUrl(null, "http://ide.example"),
    "http://ide.example/",
  );
});
...
test("applyBootstrapPayload 会写入 token、refreshToken、tenantId、projectId、theme 与 locale", () => {
  ...
  assert.equal(
    ...
    JSON.stringify("tenant-1"),
  );
  assert.equal(
    globalThis.localStorage.getItem("project_id"),
...
test("顶层正式入口即使已有本地会话，只要携带新的 handoff 仍应回跳 IDE 恢复指定工程", () => {
  ...
  assert.equal(
    plan.ideRedirectUrl,
    "http://datacenter.example:18601/?handoff=handoff-top-cache",
  );
});
