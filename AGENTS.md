...
- **默认语言**：所有交流默认使用简体中文。
- **谋定而后动**：复杂任务需先与用户讨论方案并列出计划，确认后方可编码；需求明确、影响局部、低风险的小改动直接实现。
- **简单任务免流程**：简单 UI 调整、局部排序、文案或样式修改、明确的单点修复，不创建规格文档和实施计划，不调用 brainstorming、writing-plans 或 executing-plans；查清影响后直接修改并验证。用户明确要求先讨论或出计划时除外。
...
- **无需过度兼容**: 当前项目处于开发初始阶段，所有的开发不需要考虑兼容，以最优效果为准，不论页面UI、还是数据库数据、业务交互都不需要考虑兼容

## 执行方式

简单任务直接做，不创建过程文档或实施计划。
...
- 后端只维护“从零创建当前最终结构”的建库基线，不新增版本迁移脚本、down 脚本、旧表搬迁、字段回填或启动时兼容修复逻辑。
- 开发阶段已有数据库的表、字段、约束和数据调整，直接在 WSL/Docker 当前开发数据库执行 SQL；代码库只同步更新最终建库基线。
- 服务启动时仅允许在空数据库中创建完整结构；检测到已有业务表时不得自动 ALTER、DROP、UPDATE、DELETE 或补齐历史结构。
- 修改数据库结构后，必须同时验证：空库可完整创建、当前开发数据库已直接调整、代码中不存在对应迁移适配逻辑。
- 项目处于开发初期，不保留数据库回滚迁移和历史结构兼容分支。
## 基础设施与交付
...
- 列表主体放入 `data.list`
- 列表型接口默认由后端提供分页、搜索、排序能力；前端不得为了实现分页、搜索、排序一次性拉取大批量数据再在浏览器内处理。
- 前端表格、树、清单、历史记录等可能增长的数据视图，默认按页或按需加载；确需前端全量处理时，必须先说明数据量上限和性能影响并取得确认。
- 批量新增、批量更新、批量删除、批量生成映射等操作必须优先设计后端批量接口，避免前端循环逐条请求。
- 涉及多条数据一致性的批量写入或删除，后端必须优先使用事务，确保全部成功或全部失败；不能事务化时必须说明补偿策略和失败影响。
- 批量查询大量 ID 时优先使用 POST 请求体或后端批量接口，避免把大量 ID 拼进 GET query 导致 URL 过长或请求头过大。
- 新增错误码前先复用现有规则。
